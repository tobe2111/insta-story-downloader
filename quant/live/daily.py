...
def _write_run_health(state_dir: str, kind: str, ok: list, failed: dict,
    ...
    if os.path.exists(path):
        try:
            ...
            cur = {}
    # ⚠️ **밤의 열쇠는 UTC 달력일이 아니다**(2026-09-02 실측). 아래 병합은
    #    "같은 날짜면 종목 단위로 합친다"인데, 그 날짜를 UTC로 세고 있었다.
    #    어젯밤 두 회차가 23:52 UTC → 01:05 UTC로 자정을 넘기자 두 번째
    #    회차가 첫 회차를 **덮어썼다**: 장부·패널은 22종목인데 이 기록은
    #    `runs 1 · ok 12 · not_reached 28`이었다. 하루 전에 붙인 '못 돈
    #    종목' 칸이 옛 날짜 열쇠 때문에 10종목을 부풀려 말한 것이다.
    #    밤 배치가 쓰는 열쇠(한국 달력일, `night_key`)를 그대로 쓴다 —
    #    한국 자정(15:00 UTC)은 배치 창에서 가장 먼 자리라 회차를 안 가른다.
    try:
