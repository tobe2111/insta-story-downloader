...
# 공개 챌린지 — 통합 계좌 시작금 **100만원**.
#
# ⚠️ 2026-08-14, 사장님 지적으로 8만원에서 고쳤다. 실제 장부는 이미
#    100만원이었다(2026-08-13 매칭 입금 후 원화 계좌로 재개설, 감사 211·212).
#    그런데 이 상수만 8만원에 멈춰 있었다 — **선언과 실제가 어긋난 자리**다.
#    화면·캡션은 장부의 원금을 먼저 읽게 돼 있어(감사 218) 겉으로는 안
#    드러났지만, 계좌를 새로 만드는 경로는 전부 이 값을 쓴다. 즉 장부가
#    지워지거나 새 환경에서 처음 도는 순간 8만원짜리 계좌가 생겨,
#    사이트가 말하는 원금과 조용히 달라진다.
#
#    옛 이름은 '8마일 챌린지'였다(8종목 × 만원, 영화 8 Mile 오마주). 유니버스가
#    20종목이 되고 원금이 100만원이 된 지금 그 이름은 아무것도 설명하지 못한다.
#    **지난 기록의 이름은 고치지 않는다** — 그때는 그 이름이었다.
...
def add_deposit(amount: float, memo: str = "", *, state_dir: str = "state",
    ...
    """후원 '매칭' 입금 — 통합 계좌의 원금을 늘린다 (100만 챌린지 · 100만원 → 1억).
