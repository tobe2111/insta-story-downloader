...
# ⚠️ **절 단위로** 적어 둔다(2026-09-05). 영어 번역기는 문장을 통째로 먼저
#    찾고, 못 찾으면 ` · `·` — `로 끊어 다시 찾는다. 그런데 화면에 나가는
#    문장은 이 해설 하나가 아니라 합성이라("매수 +26% — {해설} · 🏛 의회
#    운용: …"), 통째 규칙은 문장 전체가 아니라 못 맞고 절 단위로는 해설
#    **안의** ` — `에서 쪼개져 역시 못 맞았다. buy_hold 챔피언이 있는 날에만
#    나오는 문장이라 그 날이 오고서야 드러났다.
#    여기서 절 목록으로 두면 검사가 각 절이 사전에 있는지 직접 확인한다
#    (tests/test_the_site_reads_in_english_too.py).
_WHY_BUY_HOLD_CLAUSES = (
    "매매 없이 보유: 이 종목은 오디션에서 **아무것도 하지 않는 쪽**이 "
    "AI 전략을 이겼습니다(2단계 심사 통과). 매수·매도 없이 계속 들고 갑니다",
    "하락도 그대로 겪습니다",
)
