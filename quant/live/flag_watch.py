...
def _current_flags(status: dict, today: str | None = None) -> dict[str, str]:
    ...
    for key, p in (status.get("paper") or {}).items():
        ...
        # 금액이 계좌보다 큰 기록(감사 273). 레버리지가 잠긴 계좌에서는
        ...
        # 요청보다 적은 표본으로 판단한 종목(감사 266). 성적이 나쁜 것이
    ...
    # ⑥-② 재료가 **거의 비어 있다** (2026-09-07). ⑥은 열이 통째로 사라져야
    #     울린다. 그런데 열은 살아 있는데 소스가 최근 며칠치만 주는 상태가
    #     있었고, 그때 계측기는 "붙었다"고 말했다 — 실측: `x_oi_chg5`가
    #     800봉 중 31봉(3.9%) · `x_kimchi`가 200봉(25%).
    ...
    if names:
        ...
        flags["features_thin:" + ",".join(sorted(names))] = (
            f"⚠️ 재료가 거의 비어 있습니다: {len(names)}개 "
    ...
    # ⑦-④ **굶는 종목** — 이어달리기가 한 종목을 계속 뒤로 미루고 있다.
    #
    #      작업 #67(2026-09-01)이 '못 돈 종목' 칸을 만들면서 이렇게 적어
    #      두었다: *"예산이 더 조여져 한 종목이 계속 뒤로 밀리기 시작하면
    #      그때는 아무 빨간불도 안 뜬다."* **칸만 만들고 빨간불은 안 붙였다.**
    #      여기서 붙인다.
    #
    #      ⚠️ 한 밤의 `not_reached`로 울리면 안 된다 — 이어달리기는 설계상
    #         한 밤에 명단을 다 못 돌아서 그 값이 거의 매일 0이 아니다.
    #         매일 울리는 경보는 꺼진 경보와 같다(감사 99).
    #      ⚠️ 실측(2026-09-07): 최악 간격 2밤 · 중앙 1밤 · 최근 3밤 안에
    #         40종목 전부가 한 번씩 받았다. 그래서 오늘 이 경보는 **침묵한다.**
    gap = status.get("audition_gap") or {}
    starving = gap.get("starving") or {}
    if gap.get("measured") and starving:
        thr = int(gap.get("threshold") or 0)
        names = ", ".join(f"{k}({v}밤)" for k, v in
                          list(starving.items())[:5])
        more = "…" if len(starving) > 5 else ""
        flags[f"audition_starved:{gap.get('latest_night')}:{len(starving)}"] = (
            f"⚠️ 오디션이 굶는 종목: {len(starving)}종목이 {thr}밤 넘게 심사를 "
            f"받지 못했습니다 — {names}{more}. 밤 배치의 시간 예산에 걸려 "
            f"이어달리기가 이 종목들을 계속 뒤로 미루고 있다는 뜻이고, 그동안 "
            f"이 종목은 재검증 없이 옛 챔피언으로 계속 운용됩니다.")
    # ⚠️ **못 잰 것도 적는다** — 안 적으면 "굶는 종목이 없다"와 "재는 장치가
    #    고장 났다"가 화면에서 똑같이 조용하다. 이 파일이 반복해서 잡아 온
    #    바로 그 병이다.
    elif gap and not gap.get("measured"):
        flags["audition_gap_unmeasured"] = (
            f"⚠️ 오디션 간격을 재지 못했습니다 — {gap.get('why') or '사유 불명'}. "
            f"굶는 종목이 없다는 뜻이 아니라, 있는지 없는지를 모른다는 뜻입니다.")
