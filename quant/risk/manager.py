...
class RiskManager:
    ...
    def size_positions(self, df: pd.DataFrame, target: pd.Series) -> pd.Series:
        ...
        if cfg.sizing == "fixed":
            ...
            # ⚠️ **문턱이 잘못된 크기였다**(감사 209). 바로 위 주석은 원리를
            #    맞게 적어 놓고("연율 변동성이므로 크기를 안다") 문턱은
            #    **부동소수 잡음 크기**인 1e-9로 잡았다. 그래서 값이 정확히
            #    0은 아니지만 사실상 멈춘 시세가 그대로 통과했다. 실측:
            #
            #        20봉 중 19봉의 수익률이 정확히 0 · 한 봉만 0.01% 움직임
            #        → 연율 실현변동성 0.0022%  →  목표 비중 **1.0000(100%)**
            #        (같은 전략·정상 변동성 2.14%일 때는 0.6631)
            #
            #    거래정지·호가 고정·시세 피드가 마지막 값을 반복하는 날이
            #    정확히 이 모양이다. 그리고 데이터 품질 검사도 이걸 안 잡는다
            #    (실측: 위반 0건·심각도 False). 즉 **아무도 안 막고 있었다.**
            #
            #    크기로 막으려 하면 진짜 저변동성 자산(채권 ETF)까지 걸린다.
            #    구별되는 것은 크기가 아니라 **움직임의 유무**다 — 창 안의
            #    수익률이 대부분 정확히 0이면 그 변동성 추정치는 '작은 값'이
            #    아니라 **없는 값**이다. 없는 값으로 나누지 않는다.
            zero_share = (returns.fillna(0.0) == 0.0).rolling(
                cfg.vol_window).mean()
