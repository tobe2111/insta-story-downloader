...
STRATEGIES = ["ma_cross", "momentum", "mean_reversion", "rsi", "breakout", "turtle", "bollinger", "psar", "ichimoku", "dual_thrust", "supply_som", "value_anchor", "supertrend", "connors_rsi2",
...
STRATEGY_LABELS = {
    ...
    "supply_som": "수급 SOM · 외인·기관 순매수 군집",
    "value_anchor": "가치 닻 · 자기 역사 대비 저평가 보유",
    "supertrend": "슈퍼트렌드 · ATR 밴드 래칫 추세",
    "connors_rsi2": "코너스 RSI(2) · 추세 위 눌림 매수, 단기선 복귀 청산",
    "macd": "MACD 히스토그램",
    "keltner": "켈트너 채널 돌파",
    "stochastic": "스토캐스틱",
    "ml": "머신러닝 · 상승확률 예측",
...
def broadcast_json(state_dir: str = "state", with_live: bool = True) -> str:
    ...
    for a in accounts:
        ...
        if le is not None:
            ...
            # 기준: 통합 계좌는 원금(시작금 100만원+입금), 개별 계좌는 시작 만원
...
"Deposit (100만 챌린지 매칭 입금)" → Run workflow 로 직접 등록할 수 있습니다.</p>
</details></div>""")
    body = f"""<p class="kicker">Deposit</p>
<h1>매칭 입금 (100만 챌린지 · 100만원 → 1억)</h1>
...
# 방송 워터마크 QR — 사이트(100만 챌린지 기록 페이지)로 가는 다리.
...
<span id="news">100만 챌린지 — 100만원으로 시작하는 가상 자금 자동 모의투자</span>
...
<div class="u"><b>100만 챌린지 실기록</b><br>quant.jiwon-1a2.workers.dev<br>모의투자 · 수익 보장 아님</div></div>
    ...
    if(hero){{
      ...
      if(hero.principal!=null){{
        ...
        goal=`<div class="goal"><div class="bar"><div class="fill" style="width:${{Math.max(0.3,ratio*100)}}%"></div></div>
          <div class="lbl"><span>100만 챌린지 · 100만원 → 1억</span><span>${{(ratio*100).toFixed(3)}}% · 목표 1억원</span></div></div>`;
      }}
...
function showGoalScene(){{
  ...
  el.innerHTML=`<div class="box"><div class="t">100만 챌린지 · 100만원 → 1억</div>
