...
MUTATIONS = [
    ...
    ("국내 종목 코드에서 접미사만 떼고 검사를 안 한다 — 형식이 다르면 깨진다",
     ...
     "tests/tv_symbols_check.mjs"),
    ("아는 미국주식의 거래소를 생략한다 — 표가 있는데 안 쓴다",
     "docs/assets/tv-symbols.js",
    ...
    ("재생에서 킬스위치를 뗀다(폭락을 지나도 발동 기록이 없다)",
     "quant/risk/replay.py",
     "        ks = _kill_switch(ks, dd)",
     "        ks = 1.0",
     "tests/test_the_brakes_survive_a_crash_replay.py"),
    ("브레이크가 자산 곡선에 안 실린다(밟아도 낙폭이 그대로다)",
     "quant/risk/replay.py",
     "        eq_br *= 1.0 + day_r * exposure",
     "        eq_br *= 1.0 + day_r",
     "tests/test_the_brakes_survive_a_crash_replay.py"),
    ("시뮬레이션 표식을 뗀다(가상 성적이 실측처럼 읽힌다)",
     "quant/risk/replay.py",
     '        "kind": "simulation",       # ← 실측이 아니다. 읽는 쪽이 반드시 표시할 것',
     '        "kind": "live",',
     "tests/test_the_brakes_survive_a_crash_replay.py"),
    ("정직한 한계에서 비용 미반영 고지를 지운다",
     "quant/risk/replay.py",
     '            "거래 비용·슬리피지 미반영 — 낙폭·변동성 측정 목적. 수익 배수를 성과로 읽지 말 것",',
     '            "",',
     "tests/test_the_brakes_survive_a_crash_replay.py"),
    ("폭락 경로 중간의 브레이크 개입을 뗀다(끝에서 한 번 묻는 척만 한다)",
     "quant/risk/stress.py",
     "            exposure = gross_weight * ks",
     "            exposure = gross_weight",
     "tests/test_stress_scenarios_ask_the_real_brakes.py"),
    ("현금 완충을 무시한다(노출 40% 계좌에 시장 손실을 통째로 적는다)",
     "quant/risk/stress.py",
     "            eq *= 1.0 + exposure * shock",
     "            eq *= 1.0 + shock",
     "tests/test_stress_scenarios_ask_the_real_brakes.py"),
    ("환율 충격이 원화 자산까지 때린다(외화 비중이 산수에서 빠진다)",
     "quant/risk/stress.py",
     "        eq = equity * (1.0 + gross_weight * fx_share * fx_move)",
     "        eq = equity * (1.0 + gross_weight * fx_move)",
     "tests/test_stress_scenarios_ask_the_real_brakes.py"),
    ("멈춘 시세 시나리오가 브레이크가 잡는 척한다(실명이 요점인데)",
     "quant/risk/stress.py",
     '        "kill_switch_after": None,      # 못 본다 — 그것이 이 시나리오의 요점',
     '        "kill_switch_after": 1.0,',
     "tests/test_stress_scenarios_ask_the_real_brakes.py"),
    ("야간 배치에서 위험 리포트를 뺀다(만들어 두고 아무도 안 돌린다)",
     ".github/workflows/nightly-validate.yml",
     "        run: python3 scripts/risk_report.py",
     "        run: echo skip",
     "tests/test_the_brakes_survive_a_crash_replay.py"),
    ...
    # 2026-09-07 — 그림자 장부의 시계. 되감김을 정상으로 취급하면 다른
    # 계좌의 줄이 그대로 들어온다(실측: 장부 10개 × 2줄).
    ("배분 사다리가 과거로 간 봉을 받아 적는다(다른 계좌의 줄이 섞인다)",
     "quant/live/alloc_ladder.py",
    ...
    ("무제약 그림자가 같은 봉에 두 번 움직인다(멱등 해제)",
     ...
     "    if False:",
     "tests/test_the_unshackled_shadow_prices_the_brakes.py"),
    ("무제약 그림자가 과거로 간 봉을 받아 적는다(다른 계좌의 줄이 섞인다)",
     "quant/live/unshackled.py",
     "    if move == clock.BACKWARDS:",
     "    if False:",
     "tests/test_the_shadow_ledgers_hold_one_account.py"),
    ...
    # ── 고객 전략 공유 (2026-08-18) — 동의 없는 수집이 되는 순간 신뢰 붕괴 ──
    ("동의 관문을 뗀다(체크 안 해도 제작사로 전송된다 — 통보가 된다)",
     "quant/web/mystrategy.py",
     '        if str(params.get("share") or "") == "1":',
     "        if True:",
     "tests/test_the_shared_specs_reach_the_admin_desk.py"),

    ("어드민 게이트 접두사를 좁힌다(로그인 없이 고객 전략 목록이 열린다)",
     "worker.js",
    ...
    # ── 방송에 나가는 숫자 (2026-08-18 감사 288) ─────────────────────
    ("캡션 관문을 통째로 뗀다 — 계좌로 설명 안 되는 숫자가 그대로 나간다",
     "quant/reporting/social.py",
     "    why = impossible_reason(x)",
     '    why = ""',
     "tests/test_the_caption_cannot_claim_an_impossible_day.py"),
    ("하루 변동 상한을 사실상 없앤다 — +7,149%도 통과한다",
     "quant/reporting/social.py",
     "MAX_DAY_MOVE_PCT = 100.0",
     "MAX_DAY_MOVE_PCT = 1e12",
     "tests/test_the_caption_cannot_claim_an_impossible_day.py"),
    ("하루 변동 비교 자체를 건너뛴다",
     "quant/reporting/social.py",
     ...
     "    if False:",
     "tests/test_the_caption_cannot_claim_an_impossible_day.py"),
    ("장부가 스스로 단 '금액 안 맞음' 표식을 캡션이 무시한다",
     "quant/reporting/social.py",
     '    bad = x.get("impossible_amounts")',
     "    bad = None",
     "tests/test_the_caption_cannot_claim_an_impossible_day.py"),
    ("정정문이 있어도 포인터에 싣지 않는다 — 되돌린 글이 '오늘 올릴 글'로 나온다",
     "quant/reporting/social.py",
     "    if os.path.exists(os.path.join(out_dir, CORRECTION_FILE)):",
     "    if False:",
     "tests/test_the_caption_cannot_claim_an_impossible_day.py"),
    ("어드민이 정정 경고를 그리지 않는다 — 사람이 그대로 복사해 올린다",
     "docs/admin.html",
     "    if(d.correction){",
     "    if(false){",
     "tests/test_the_caption_cannot_claim_an_impossible_day.py"),
    ...
    # ── 13F 저명 투자자 확신 오버레이 (2026-09-22 감사 328) ──────────
    # 오버레이는 '이미 사겠다고 한 신호'만 키운다 — 단독 트리거가 아니다.
    ("오버레이의 관망·팔자 보호를 뗀다 — 겹쳐 담기가 팔자를 매수 쪽으로 민다",
     "quant/live/thirteenf_overlay.py",
     "    if x <= 0.0:",
     "    if False:",
     "tests/test_famous_investors_are_a_slow_overlay_not_a_trigger.py"),
    ("오버레이 상한을 뗀다 — 몫의 비율이 100%를 넘어 과대 베팅이 된다",
     "quant/live/thirteenf_overlay.py",
     "    return min(1.0, x)",
     "    return x",
     "tests/test_famous_investors_are_a_slow_overlay_not_a_trigger.py"),
    ("겹친 수와 무관하게 항상 최대 보너스를 준다 — '1명'도 '3명'과 같아진다",
     "quant/live/thirteenf_overlay.py",
     "    return 1.0 + max_bonus * frac",
     "    return 1.0 + max_bonus",
     "tests/test_famous_investors_are_a_slow_overlay_not_a_trigger.py"),
    ("한 제출자의 중복 보유를 여러 표로 센다 — 겹쳐 담기 개수가 부풀려진다",
     "quant/data/thirteenf.py",
     "            if sym is None or sym in seen:",
     "            if sym is None:",
     "tests/test_famous_investors_are_a_slow_overlay_not_a_trigger.py"),
    ("CUSIP 매칭을 끈다 — 이름만 다른 같은 종목을 못 잡아 겹치기를 놓친다",
     "quant/data/thirteenf.py",
     "        if cus and cus in {_norm_cusip(c) for c in spec.get(\"cusips\", [])}:",
     "        if False and cus in {_norm_cusip(c) for c in spec.get(\"cusips\", [])}:",
     "tests/test_famous_investors_are_a_slow_overlay_not_a_trigger.py"),
    ("미국 트랙에서 오버레이 적용을 무력화한다 — 겹쳐 담기가 신호에 안 닿는다",
     "quant/live/intraday_us.py",
     "        signals[sym] = apply_overlay(signals[sym], sym, cluster, max_bonus=bonus)",
     "        signals[sym] = signals[sym]",
     "tests/test_famous_investors_are_a_slow_overlay_not_a_trigger.py"),

    # ── 13F 오버레이 세기를 기계가 정한다 (2026-09-23 감사 329) ──────
    ("증거가 약한데도 세기를 올린다 — 관측 부족·낮은 t·음수 차이를 무시한다",
     "quant/live/thirteenf_overlay.py",
     "    if not (md > 0.0) or t < EDGE_T_GATE:",
     "    if False:",
     "tests/test_the_machine_sets_the_13f_weight.py"),
    ("관측이 모자라도 세기를 정한다 — '아직 모른다'를 큰 확신으로 읽는다",
     "quant/live/thirteenf_overlay.py",
     "    if n < MIN_EDGE_OBS:",
     "    if False:",
     "tests/test_the_machine_sets_the_13f_weight.py"),
    ("앞선 성적의 부호를 뒤집는다 — 오른 종목을 내렸다고 잰다(세기 근거 오염)",
     "quant/live/thirteenf_tune.py",
     "    return p1 / p0 - 1.0",
     "    return p0 / p1 - 1.0",
     "tests/test_the_machine_sets_the_13f_weight.py"),
    ...
    # ── 트레이딩뷰 표가 유니버스를 못 따라간 구멍 (2026-08-25 CI가 적발) ──
    ("고정 코어 ETF의 거래소가 틀린다(잔고 1위를 눌렀는데 엉뚱한 지정)",
     "docs/assets/tv-symbols.js",
     '    UUP: "AMEX",       // 달러 — NYSE Arca',
     '    UUP: "KRX",        // 달러 — NYSE Arca',
     "tests/tv_symbols_check.mjs"),
    ("티커 꼴 검사 없이 후퇴한다(소문자·특수문자도 심볼로 지어낸다)",
     "docs/assets/tv-symbols.js",
     "      return /^[A-Z][A-Z0-9]{0,4}$/.test(symbol) ? symbol : null;",
     "      return symbol;",
     "tests/tv_symbols_check.mjs"),
    ("스냅샷을 무시하고 언제나 고정 20종목을 돌려준다(확장이 유령이 된다)",
     "quant/universe.py",
    ...
    ("현금을 모르면 0원으로 친다('모름'이 '빈 지갑'이 된다)",
     ...
     "    if (false) {",
     "tests/test_the_tracks_show_live_equity.py"),
    ("봇이 현금을 스냅샷에 남기지 않는다(프로그램 실시간이 원리적으로 죽는다)",
     "quant/live/multi.py",
     '            "cash": cash,',
     "",
     "tests/test_the_program_shows_live_equity.py"),
    ("조종석이 자기만의 계산을 시작한다(사이트와 규칙이 갈라진다)",
     "quant/web/app.py",
     '        _inline_asset("track-live.js") + _MONITOR_JS + "</body>", 1)',
     '        _MONITOR_JS + "</body>", 1)',
     "tests/test_the_program_shows_live_equity.py"),

    # ── 트랙 실시간 평가 (2026-08-23 사장님: "실시간으로 자산 계속") ────
    ("일부만 받아도 합계를 낸다(계좌가 실제보다 작게 보인다)",
     "docs/assets/track-live.js",
     "      equityLive: complete ? Number(equity) + delta : null,",
     "      equityLive: Number(equity) + delta,",
     "tests/test_the_tracks_show_live_equity.py"),
    ("숏 부호를 무시한다(선물 페이지가 오르면 버는 것으로 그린다)",
     "docs/assets/track-live.js",
     '    return h.direction === "short" ? -q : q;',
     "    return q;",
     "tests/test_the_tracks_show_live_equity.py"),
    ("실시간 줄이 들어갈 자리가 사라진다(다시 확정값만 남는다)",
     "docs/intraday.html",
     "    '<div id=\"tl-line\" style=\"margin-top:8px;font-size:16px\"></div>'+",
     "    ''+",
     "tests/test_the_tracks_show_live_equity.py"),
    ...
    ("코드표 갱신 실패를 오늘 도장으로 찍는다(30일간 재시도가 죽는다)",
     ...
     "tests/test_the_korean_calendar_arrives_by_key.py"),
    ("서버 오류 메시지의 키를 가리지 않는다(로그로 키가 샌다)",
     "quant/data/earnings.py",
     '            msg = str(js.get("message", ""))[:120].replace(key, "***")',
     '            msg = str(js.get("message", ""))[:120]',
     "tests/test_the_korean_calendar_arrives_by_key.py"),
    ("접수일을 ISO로 바꾸지 않는다(붙일 때 날짜가 영원히 안 맞는다)",
     "quant/data/earnings.py",
     '                out.add(f"{rd[:4]}-{rd[4:6]}-{rd[6:]}")',
     "                out.add(rd)",
     "tests/test_the_korean_calendar_arrives_by_key.py"),
    ("형식 불량 접수일을 삼킨다(쓰레기가 발표일이 된다)",
     "quant/data/earnings.py",
     '            if len(rd) == 8 and rd.isdigit():',
     "            if rd:",
     "tests/test_the_korean_calendar_arrives_by_key.py"),
    ("키가 없어도 한국을 묻는다(매일 실패 경고가 진짜 고장을 가린다)",
     "quant/live/daily.py",
     "                if dart_key() is not None:",
     "                if True:",
     "tests/test_the_korean_calendar_arrives_by_key.py"),
    ("가림 패턴이 DART 키를 모른다(오류 로그 URL로 키가 샌다)",
     "quant/utils/http.py",
     '    r"crtfc[_-]?key|"    # DART 인증키 — 이름이 일반 패턴 밖이라 명시(2026-08-23)',
     '    r""',
     "tests/test_the_korean_calendar_arrives_by_key.py"),

    # ── 달력 수급 후보 — 만기 주간·FOMC 표류 (2026-08-23) ─────────────
    ("만기 '주간'이 월 후반 전체가 된다(수급 창이 흐려진다)",
     "quant/strategies/expiry_week.py",
     "            pos[i] = 1.0 if tf - 4 <= days[i] <= tf else 0.0",
     "            pos[i] = 1.0 if tf - 4 <= days[i] else 0.0",
     "tests/test_the_calendar_tells_who_must_trade.py"),
    ("셋째 금요일 계산이 15일로 고정된다(달력이 틀린 달이 생긴다)",
     "quant/strategies/expiry_week.py",
     "    return 15 + (4 - w15) % 7",
     "    return 15",
     "tests/test_the_calendar_tells_who_must_trade.py"),
    ("발표일 당일에도 산다(발표 전 표류가 발표 후 반응까지 먹는다)",
     "quant/strategies/fomc_drift.py",
     "            if nxt1 in FOMC_DECISION_DAYS or nxt2 in FOMC_DECISION_DAYS:",
     "            if d.isoformat() in FOMC_DECISION_DAYS \\\n"
     "                    or nxt1 in FOMC_DECISION_DAYS \\\n"
     "                    or nxt2 in FOMC_DECISION_DAYS:",
     "tests/test_the_calendar_tells_who_must_trade.py"),
    ("달력 수명이 끝나도 조용하다(관망이 고장을 숨긴다)",
     "quant/strategies/fomc_drift.py",
     "        if stale:",
     "        if False:",
     "tests/test_the_calendar_tells_who_must_trade.py"),
    ("FOMC 후보가 링에 서지 않는다(전략은 있는데 심사를 안 받는다)",
     "quant/live/retrain.py",
     '        {"strategy": "fomc_drift", "params": {}},',
     '        # {"strategy": "fomc_drift", "params": {}},',
     "tests/test_the_calendar_tells_who_must_trade.py"),

    # ── 다양성 가중 그림자 — 섞는 비중 축 (2026-08-23) ────────────────
    ("1석 의회도 혼합 쌍을 낸다(비교가 성립하지 않는 것을 잰다)",
     "quant/live/diversity_shadow.py",
     "        if len(members) < 2:\n            return None",
     "        if False:\n            return None",
     "tests/test_the_shadow_that_respects_correlation.py"),
    ("잴 것이 없는 날도 기록한다(빈 회차가 가짜 평평함을 만든다)",
     "quant/live/diversity_shadow.py",
     "    if not usable:\n        return None",
     "    if False:\n        return None",
     "tests/test_the_shadow_that_respects_correlation.py"),
    ("다양성 트랙이 실제 비중을 쓴다(두 트랙이 같은 계좌가 된다)",
     "quant/live/diversity_shadow.py",
     "        target = {k: pair[ti] / n for k, pair in usable.items()}",
     "        target = {k: pair[0] / n for k, pair in usable.items()}",
     "tests/test_the_shadow_that_respects_correlation.py"),
    ("전일 수익 반영을 잊는다(계좌가 영원히 수수료만 낸다)",
     "quant/live/diversity_shadow.py",
     "                ret += float(w) * (float(p1) / float(p0) - 1.0)",
     "                ret += 0.0",
     "tests/test_the_shadow_that_respects_correlation.py"),
    ("일일 배치가 그림자를 전진시키지 않는다(계좌는 있는데 영원히 빈다)",
     "quant/live/daily.py",
    ...
    # ── 실적 발표 후 표류(PEAD) — 가설 우선 2호 (2026-08-23) ──────────
    ("발표일 아닌 날의 급등에도 산다(뉴스 아닌 변동을 뉴스로 친다)",
     "quant/strategies/pead.py",
     "            if i > 0 and earn[i] > 0 and ret[i] > self.min_jump:",
     "            if i > 0 and ret[i] > self.min_jump:",
     "tests/test_the_drift_after_the_news_is_a_hypothesis.py"),
    ("발표일을 모르는 시장에서 0으로 지어낸다('몰랐다'가 '발표 없음'이 된다)",
     "quant/strategies/pead.py",
     '        if "earn_day" not in df.columns or n < 2:',
     "        if False:",
     "tests/test_the_drift_after_the_news_is_a_hypothesis.py"),
    ("보유 창이 안 끝난다(발표 한 번에 영원히 들고 있는다)",
     "quant/strategies/pead.py",
     "            elif left > 0:\n                left -= 1",
     "            elif left > 0:\n                pass",
     "tests/test_the_drift_after_the_news_is_a_hypothesis.py"),
    ("발표일 캐시에 없는 종목을 0으로 채운다(모름이 사실로 둔갑한다)",
     "quant/data/earnings.py",
     "        if not dates:\n            return df",
     "        if False:\n            return df",
     "tests/test_the_drift_after_the_news_is_a_hypothesis.py"),
    ("PEAD 재료를 파이프라인이 안 붙인다(전략은 있는데 영원히 관망한다)",
     "quant/live/retrain.py",
    ...
    ("월말 창을 뒤집는다(월 중순에 사고 월말에 관망 — 가설의 반대)",
     ...
     "        pos = ((day < self.entry_day) & (day > self.exit_day)).astype(float)",
     "tests/test_the_calendar_bet_knows_why_it_exists.py"),
    ("달력 창 검사를 뺀다(진입 25 < 청산 3 같은 설정이 통과한다)",
     "quant/strategies/turn_of_month.py",
     "        if not (exit_day < entry_day and 1 <= exit_day and entry_day <= 31):",
     "        if False:",
     "tests/test_the_calendar_bet_knows_why_it_exists.py"),
    ("월말 후보를 링에서 뺀다(구현만 하고 안 세우면 없는 것과 같다)",
     "quant/live/retrain.py",
    ...
    ("같은 봉을 다시 돌리면 기록이 늘어난다(하루가 두 줄이 된다)",
     ...
     "        if False:",
     "tests/test_the_size_of_the_bet_is_finally_measured.py"),
    ("사이징 사다리가 과거로 간 봉을 받아 적는다(다른 계좌의 줄이 섞인다)",
     "quant/live/sizing_ladder.py",
     "        if move == clock.BACKWARDS:",
     "        if False:",
     "tests/test_the_shadow_ledgers_hold_one_account.py"),
    ...
    ("재료가 그대로인데도 같은 봉을 다시 돌린다(그날 매매·비용이 두 배가 된다)",
     ...
     "    if False:",
     "tests/test_the_first_run_of_the_day_does_not_seal_it.py"),
    ("검증 장부 지문에서 종목 수를 뺀다(같은 날 2종목→20종목을 못 알아본다)",
     "quant/live/redo.py",
     '        return f"{asof}/{len(val)}"',
     '        return f"{asof}"',
     "tests/test_the_first_run_of_the_day_does_not_seal_it.py"),
    ("다른 봉의 되돌림 지점으로 오늘을 되감는다(어제 잔고로 오늘을 계산한다)",
     "quant/live/redo.py",
     '    if str(point.get("bar") or "") != str(bar) or not point.get("state"):\n        return False\n',
     "    if False:\n        return False\n",
     "tests/test_the_first_run_of_the_day_does_not_seal_it.py"),
    ("되돌림 지점을 얕게 복사한다(원본이 바뀌면 지점도 같이 바뀐다)",
     "quant/live/redo.py",
     "    snap = {k: copy.deepcopy(v) for k, v in st.items() if k not in SKIP_KEYS}",
     "    snap = {k: v for k, v in st.items() if k not in SKIP_KEYS}",
     "tests/test_the_first_run_of_the_day_does_not_seal_it.py"),
    ("어떤 검증 장부를 썼는지 기록에 안 남긴다(다음 실행이 판단할 근거를 잃는다)",
     "quant/live/daily.py",
     '              "validation_stamp": val_stamp,',
     "",
     "tests/test_the_first_run_of_the_day_does_not_seal_it.py"),

    # ── 통과 0종목인 날, 왜 그런지까지 말한다 (2026-08-19 검증 첫 완주) ──
    ("화면의 통과선을 몰래 낮춘다(판정선과 갈라져 화면이 거짓말을 한다)",
     "quant/live/provable.py",
     "DSR_PASS = 0.95            # validation_gate 와 같은 선(여기서 바꾸면 안 된다)",
     "DSR_PASS = 0.50",
     "tests/test_the_bar_is_out_of_reach_and_we_say_so.py"),
    ("필요 샤프가 표본 길이를 안 본다(장부 대신 고정값을 발표한다)",
     "quant/live/provable.py",
     "        std = math.sqrt(max(1e-12, den * den / (T - 1.0)))",
     "        std = math.sqrt(max(1e-12, den * den / 799.0))",
     "tests/test_the_bar_is_out_of_reach_and_we_say_so.py"),
    ("다중검정 보정을 뺀다(많이 시도해도 문턱이 안 오른다)",
     "quant/live/provable.py",
     "        star = expected_max_sharpe(n_trials, std) if n_trials > 1 else 0.0",
     "        star = 0.0",
     "tests/test_the_bar_is_out_of_reach_and_we_say_so.py"),
    ("봉이 세 개뿐인 기록으로도 필요 샤프를 발표한다",
     "quant/live/provable.py",
     "    if T < 4:",
     "    if False:",
     "tests/test_the_bar_is_out_of_reach_and_we_say_so.py"),
    ("장부가 비었는데 숫자를 지어낸다(빈칸이 '측정됨'으로 읽힌다)",
     "quant/live/provable.py",
    ...
    # ── 비용을 세고 보여 준다 (2026-08-19 사장님 질문) ──────────────
    ('그냥 보유가 사는 값을 안 문다(실험만 비용을 무는 비교로 되돌아간다)',
     'quant/live/intraday_challenger.py',
     '    return round(((1.0 - float(per_side)) * (1.0 + gross) - 1.0) * 100, 4)',
     '    return round(gross * 100, 4)',
     'tests/test_the_costs_are_counted_and_shown.py'),
    ('편도 비용률을 0으로 만든다(비용이 없는 세상으로 계좌가 굴러간다)',
     'quant/backtest/costs.py',
     '        return float(self.fee + self.slippage)',
     '        return 0.0',
     'tests/test_the_costs_are_counted_and_shown.py'),
    ('브로커가 낸 수수료를 안 센다(얼마 냈는지 다시 알 수 없게 된다)',
     'quant/broker/paper.py',
     '        self.fee_paid += fee',
     '        self.fee_paid += 0.0',
     'tests/test_the_costs_are_counted_and_shown.py'),
    ('되짚기가 기록이 부인한 체결까지 센다(안 낸 비용이 장부에 생긴다)',
     'quant/live/ledger_costs.py',
     '        if key in denied:\n            continue',
     '        if False:\n            continue',
     'tests/test_the_costs_are_counted_and_shown.py'),
    ('씨앗이 실제로 센 값을 덮어쓴다(추정이 사실을 밀어낸다)',
     'quant/live/ledger_costs.py',
     '    if st.get("cost_paid") is not None:\n        return {}',
     '    if False:\n        return {}',
     'tests/test_the_costs_are_counted_and_shown.py'),
    ('장부가 오늘 낸 비용을 0으로 적는다(누적이 영영 안 자란다)',
     'quant/live/daily.py',
     '    cost_today = round(float(getattr(broker, "fee_paid", 0.0) or 0.0), 2)',
     '    cost_today = 0.0',
     'tests/test_the_costs_are_counted_and_shown.py'),
    ('첫 화면이 이미 낸 비용을 안 그린다(사장님 질문에 다시 답할 자리가 없어진다)',
     'docs/index.html',
    ...
    ('화면이 숏 수량을 음수 그대로 그린다(읽는 사람에게 뜻이 없다)',
     ...
     '        q.toFixed(6)+',
     'tests/test_the_short_side_is_really_a_short.py'),
    ('규칙이 바뀐 날을 리포트가 안 싣는다(곡선이 왜 달라졌는지 모른다)',
     'quant/live/futures_challenger.py',
     '        "rule_changes": list(RULE_CHANGES),',
     '        "rule_changes": [],',
     'tests/test_the_short_side_is_really_a_short.py'),
    ('화면이 규칙 변경을 안 그린다(조용한 골대 이동이 된다)',
     'docs/futures.html',
     '    (window.QuantTrades?QuantTrades.ruleChangeItems(d.rule_changes):"")+',
     '    ""+',
     'tests/test_the_short_side_is_really_a_short.py'),
    ('화면에서 배율 설명을 통째로 지운다(매일 최대치를 태우는 것으로 읽힌다)',
     'docs/futures.html',
     "    /*lev*/'<li>",
     "    /*lev*/''+'<!--",
     'tests/test_the_short_side_is_really_a_short.py'),
    ('배율을 확신과 무관하게 항상 최대로 태운다(모르는 날에 크게 건다)',
     'quant/live/futures_challenger.py',
     '    return 1.0 + (top - 1.0) * conviction',
     '    return top',
     'tests/test_the_short_side_is_really_a_short.py'),
    ('배율이 확신을 아예 안 본다(1배에 고정 — 사장님 지시가 무효가 된다)',
     'quant/live/futures_challenger.py',
     '        conviction = abs(float(signal))',
     '        conviction = 0.0',
     'tests/test_the_short_side_is_really_a_short.py'),
    ('신호가 없어도 최대 배율로 태운다(모르는 날이 가장 큰 베팅이 된다)',
     'quant/live/futures_challenger.py',
     '    except (TypeError, ValueError):\n        return 1.0',
     '    except (TypeError, ValueError):\n        return float(max_leverage)',
     'tests/test_the_short_side_is_really_a_short.py'),
    ('체결이 배율을 안 쓴다(확신이 커도 크기가 안 변한다)',
     'quant/live/futures_challenger.py',
     '        target = slice_budget * float(sig) * leverage_for(sig, max_gross)',
     '        target = slice_budget * float(sig)',
     'tests/test_the_short_side_is_really_a_short.py'),
    ('청산을 아예 안 한다(현실에 없는 성적이 나온다)',
     'quant/live/futures_challenger.py',
     '    if ratio is None or ratio > float(rate):\n        return None',
     '    return None\n    if ratio is None or ratio > float(rate):\n        return None',
     'tests/test_the_short_side_is_really_a_short.py'),
    ('건강한 계좌까지 청산한다(조금 잃어도 털어 버린다)',
     'quant/live/futures_challenger.py',
     '    ratio = margin_ratio(st, prices)\n    if ratio is None or ratio > float(rate):',
     '    ratio = margin_ratio(st, prices)\n    if ratio is None:',
     'tests/test_the_short_side_is_really_a_short.py'),
    ('증거금률을 순노출로 잰다(롱·숏이 상쇄돼 위험이 안 보인다)',
     'quant/live/futures_challenger.py',
     '    gross = gross_exposure(st, prices)\n    if gross <= 0:\n        return None',
     '    gross = abs(sum(float(q) * float(prices.get(k) or 0)\n                    for k, q in (st.get("positions") or {}).items()))\n    if gross <= 0:\n        return None',
     'tests/test_the_short_side_is_really_a_short.py'),
    ('청산을 새 판단 뒤로 미룬다(털렸어야 할 계좌가 한 회차를 더 버틴다)',
     'quant/live/futures_challenger.py',
     '    liq = liquidation_check(st, prices)\n    if liq:',
     '    liq = None\n    if liq:',
     'tests/test_the_short_side_is_really_a_short.py'),
    ('리포트가 증거금률·청산을 안 싣는다(화면이 위험을 못 그린다)',
     'quant/live/futures_challenger.py',
     '        "liquidations": int(st.get("liquidations") or 0),\n        "liquidated": last.get("liquidated"),',
     '        "liquidations": 0,\n        "liquidated": None,',
     'tests/test_the_short_side_is_really_a_short.py'),
    ('화면에서 청산 설명을 통째로 지운다(많이 잃어도 계좌는 남는 줄 안다)',
     'docs/futures.html',
     "    /*liq*/'<li>",
     "    /*liq*/''+'<!--",
     'tests/test_the_short_side_is_really_a_short.py'),
    ('화면이 확신을 검증된 것처럼 적는다(배율이 우위처럼 읽힌다)',
     'docs/futures.html',
     "      '<b>다만 그 확신이 실제로 맞는지는 이 트랙이 아직 증명하지 '+",
     "      '<b>'+",
     'tests/test_the_short_side_is_really_a_short.py'),
    ("잴 것이 없는 날 증거금률을 0%로 그린다('청산 직전'으로 읽힌다)",
     'docs/futures.html',
     '        (d.margin_ratio==null?\'<span class="sub">—</span>\'',
     '        (false?\'<span class="sub">—</span>\'',
     'tests/test_the_short_side_is_really_a_short.py'),
    ('청산이 일어나도 요약이 조용하다(가장 큰 사건이 안 보인다)',
     'docs/futures.html',
     "    (d.liquidated\n      ?'<div class=\"warn\"><b class=\"down\">💀 강제 청산됐습니다.</b> 증거금률이 '+",
     "    (false\n      ?'<div class=\"warn\"><b class=\"down\">💀 강제 청산됐습니다.</b> 증거금률이 '+",
     'tests/test_the_short_side_is_really_a_short.py'),
    ('화면에서 자금조달 설명을 통째로 지운다(안 무는 것으로 읽힌다)',
     'docs/futures.html',
     "    /*fund*/'<li>",
     "    /*fund*/''+'<!--",
     'tests/test_the_short_side_is_really_a_short.py'),
    ...
    # ── "투자를 하는 모습"이 보이는가 (2026-08-23 감사 309) ──────────
    ('숏 평가금액을 부호 그대로 더한다(롱과 상쇄돼 굴리는 돈이 사라진다)',
     'quant/live/holdings.py',
     '            gross += abs(float(v))',
     '            gross += float(v)',
     'tests/test_every_track_shows_what_it_bought.py'),
    ('시세를 못 받은 줄을 조용히 넘긴다(굴리는 비중이 실제보다 작게 나온다)',
     'quant/live/holdings.py',
     '        if v is None:\n            unknown += 1\n            continue',
     '        if v is None:\n            continue',
     'tests/test_every_track_shows_what_it_bought.py'),
    ('자산을 모를 때도 비중을 계산한다(0으로 나누거나 지어낸다)',
     'quant/live/holdings.py',
     '    if not (eq > 0):\n        return None',
     '    if False:\n        return None',
     'tests/test_every_track_shows_what_it_bought.py'),
    ('미국 리포트가 굴리는 비중을 안 싣는다(수익률만 남아 시드 전부로 읽힌다)',
     'quant/live/intraday_us.py',
     '        "deployed": _deployed(st, eq),',
     '        "deployed": None,',
     'tests/test_every_track_shows_what_it_bought.py'),
    ('코인 리포트가 굴리는 비중을 안 싣는다',
     'quant/live/intraday_challenger.py',
     '        "deployed": _deployed(st, eq),',
     '        "deployed": None,',
     'tests/test_every_track_shows_what_it_bought.py'),
    ('선물 리포트가 굴리는 비중을 안 싣는다',
     'quant/live/futures_challenger.py',
     '        "deployed": _deployed(st, eq),',
     '        "deployed": None,',
     'tests/test_every_track_shows_what_it_bought.py'),
    ('체결 표가 실현 손익을 늘 비운다(판 값이 이익인지 손절인지 안 보인다)',
     'docs/assets/trades.js',
     '      var known = (r != null && isFinite(Number(r)));',
     '      var known = false;',
     'tests/test_every_track_shows_what_it_bought.py'),
    ('체결이 없을 때 표가 아무 말도 안 한다("없다"와 "아직"이 구별 안 된다)',
     'docs/assets/trades.js',
     "        '아직 체결이 없습니다</td></tr>';",
     "        '</td></tr>';",
     'tests/test_every_track_shows_what_it_bought.py'),
    ('굴리는 중인데 관망이라 설명한다(대조군 — 없는 이유를 붙인다)',
     'docs/assets/trades.js',
     '    var idle = pct < 1.0;',
     '    var idle = true;',
     'tests/test_every_track_shows_what_it_bought.py'),
    ('전량 현금인 이유를 화면이 설명하지 않는다(정상이 고장으로 읽힌다)',
     'docs/assets/trades.js',
     '              \'안 삽니다. 전량 현금은 고장이 아니라 "지금은 살 이유가 \' +',
     "              '안 삽니다. ' +",
     'tests/test_every_track_shows_what_it_bought.py'),
    ('미국 페이지에서 체결 표를 없앤다(48번 사고팔아도 화면엔 숫자뿐)',
     'docs/us.html',
     '<div class="card tw"><table id="tr">',
     '<div class="card tw"><table id="tr-none">',
     'tests/test_every_track_shows_what_it_bought.py'),
    ('미국 페이지가 마지막으로 판단한 시각을 안 적는다(왜 멈췄는지 모른다)',
     'docs/us.html',
     "    '<div class=\"sub\" id=\"last-round\" style=\"margin-top:4px\">마지막 회차: <b>'+",
     "    '<div class=\"sub\" id=\"last-round\" style=\"margin-top:4px\"><b>'+",
     'tests/test_every_track_shows_what_it_bought.py'),

    # ── 확신도 눈금이 모델이 사는 자리까지 닿는가 (2026-08-23 감사 310) ──
    ('눈금의 끝을 옛 자리(확률 1.00)로 되돌린다(자산의 3%만 굴리게 된다)',
     'quant/live/conviction.py',
     'FULL_CONVICTION_PROB = 0.70',
     'FULL_CONVICTION_PROB = 1.00',
     'tests/test_the_dial_reaches_where_the_model_lives.py'),
    ('문턱이 눈금 끝보다 높아도 계산을 밀어붙인다(배수가 음수 — 사려던 것을 판다)',
     'quant/live/conviction.py',
     '    if not (old_span > 0.0) or not (new_span > 0.0):\n        return 1.0',
     '    if False:\n        return 1.0',
     'tests/test_the_dial_reaches_where_the_model_lives.py'),
    ('이진 사이징에도 배수를 곱한다(0/1 신호에 없는 눈금을 만든다)',
     'quant/live/conviction.py',
     '    if str(params.get("sizing") or "proba") != "proba":\n        return 1.0',
     '    if False:\n        return 1.0',
     'tests/test_the_dial_reaches_where_the_model_lives.py'),
    ('규칙 전략의 신호에도 배수를 곱한다(확률이 아닌 값을 확률처럼 키운다)',
     'quant/live/conviction.py',
     '    if not isinstance(spec, dict) or spec.get("strategy") != "ml":\n        return 1.0',
     '    if False:\n        return 1.0',
     'tests/test_the_dial_reaches_where_the_model_lives.py'),
    ('롱 전용 트랙에서도 음수를 남긴다(체결 규칙이 숏을 연다)',
     'quant/live/conviction.py',
     '    lo = -1.0 if allow_short else 0.0',
     '    lo = -1.0',
     'tests/test_the_dial_reaches_where_the_model_lives.py'),
    ("NaN 신호를 그대로 통과시킨다('모른다'가 숫자가 된다)",
     'quant/live/conviction.py',
     '    if x != x:                          # NaN — 모른다는 뜻이다. 지어내지 않는다.\n        return None',
     '    if False:\n        return None',
     'tests/test_the_dial_reaches_where_the_model_lives.py'),
    ('문턱이 없는 객체도 ML로 넘겨짚는다(규칙 전략에 배수가 붙는다)',
     'quant/live/conviction.py',
     '    if not hasattr(strategy, "threshold") or not hasattr(strategy, "sizing"):\n        return {}',
     '    if False:\n        return {}',
     'tests/test_the_dial_reaches_where_the_model_lives.py'),
    ('미국 트랙이 옛 눈금으로 되돌아간다(계산만 맞고 아무것도 안 바뀐다)',
     'quant/live/intraday_us.py',
     '        signals[sym] = recalibrate(sig, spec_of(strat))   # 숏·레버리지 없음',
     '        signals[sym] = max(0.0, min(1.0, sig))',
     'tests/test_the_dial_reaches_where_the_model_lives.py'),
    ('코인 트랙만 옛 눈금으로 남는다(트랙 비교가 눈금 비교로 오염된다)',
     'quant/live/intraday_challenger.py',
     '        signals[sym] = recalibrate(sig, spec_of(strat))   # 숏·레버리지 없음',
     '        signals[sym] = max(0.0, min(1.0, sig))',
     'tests/test_the_dial_reaches_where_the_model_lives.py'),
    ('선물 트랙만 옛 눈금으로 남는다',
     ...
     'tests/test_the_dial_reaches_where_the_model_lives.py'),
    ('적용된 배수를 장부에 안 남긴다(장치가 꺼져도 아무도 모른다)',
     'quant/live/intraday_us.py',
     '    if any(v != 1.0 for v in (scales or {}).values()):\n        rec["conviction_scale"] = {k: round(float(v), 4)\n                                   for k, v in scales.items()}',
     '    if False:\n        rec["conviction_scale"] = {}',
     'tests/test_the_dial_reaches_where_the_model_lives.py'),
    ('본 계좌의 공용 사이저까지 새 눈금으로 바꾼다(실제 원금의 위험이 커진다)',
     'quant/strategies/ml.py',
     '        span = max(1e-9, 0.5 - gate)           # 경계~확신(1.0)까지의 폭',
     '        span = max(1e-9, 0.20 - gate)',
     'tests/test_the_dial_reaches_where_the_model_lives.py'),
    ('규칙이 바뀐 사실을 미국 장부가 안 싣는다(조용한 골대 이동)',
     'quant/live/intraday_us.py',
     '        "rule_changes": [CONVICTION_RULE_CHANGE, OVERLAY_13F_RULE],',
     '        "rule_changes": [],',
     'tests/test_the_dial_reaches_where_the_model_lives.py'),
    ('규칙이 바뀐 사실을 코인 장부가 안 싣는다',
     'quant/live/intraday_challenger.py',
     '        "rule_changes": [CONVICTION_RULE_CHANGE],',
     '        "rule_changes": [],',
     'tests/test_the_dial_reaches_where_the_model_lives.py'),
    ('선물 장부가 눈금 재보정을 규칙 변경으로 안 적는다(배율만 적고 만다)',
     'quant/live/futures_challenger.py',
     'RULE_CHANGES.append(_CONVICTION_RULE_CHANGE)',
     'pass',
     'tests/test_the_dial_reaches_where_the_model_lives.py'),
    ('바뀐 규칙을 화면이 안 그린다(장부에만 있고 아무도 못 본다)',
     'docs/assets/trades.js',
     '  function ruleChangeItems(changes) {',
     '  function ruleChangeItems(changes) {\n    return "";',
     'tests/test_the_dial_reaches_where_the_model_lives.py'),
    ('미국 페이지가 규칙 변경을 한계 목록에 안 끼운다',
     'docs/us.html',
     '    if(items){const ul=document.getElementById("limits");\n              ul.innerHTML=items+ul.innerHTML;}',
     '    if(false){}',
     'tests/test_the_dial_reaches_where_the_model_lives.py'),
    ('코인 페이지가 규칙 변경을 한계 목록에 안 끼운다',
     'docs/intraday.html',
     '    (window.QuantTrades?QuantTrades.ruleChangeItems(d.rule_changes):"")+',
     '    ""+',
     'tests/test_the_dial_reaches_where_the_model_lives.py'),

    # ── 머신러닝 성적표가 정직한가 (2026-08-24 감사 311) ────────────
    ("얇은 표본에서도 '우연을 이겼다'고 말한다(운을 실력으로 판다)",
     'quant/reporting/ml_health.py',
     '        "beats_chance": bool(n and lo is not None and lo > 0.5),',
     '        "beats_chance": bool(n),',
     'tests/test_the_model_report_card_is_honest.py'),
    ("얇은 표본에서도 '우연보다 못한다'고 단정한다(인상을 통계라 부른다)",
     'quant/reporting/ml_health.py',
     '        "worse_than_chance": bool(n and hi is not None and hi < 0.5),',
     '        "worse_than_chance": bool(n),',
     'tests/test_the_model_report_card_is_honest.py'),
    ('연습(인샘플) 성적을 실전 숫자에 섞는다(좋은 쪽이 나쁜 쪽을 가린다)',
     'quant/reporting/ml_health.py',
     '        rate, cnt = last.get("live_hit"), last.get("live_hit_n")',
     '        rate, cnt = last.get("hit_rate"), last.get("hit_n")',
     'tests/test_the_model_report_card_is_honest.py'),
    ('통합 계좌를 종목별 모델로 세어 넣는다(이중 계산)',
     'quant/reporting/ml_health.py',
     '            if not os.path.basename(f).startswith("portfolio")]',
     '            if True]',
     'tests/test_the_model_report_card_is_honest.py'),
    ('기록이 없을 때 적중률을 0%로 적는다(모르는 것이 실패가 된다)',
     'quant/reporting/ml_health.py',
     '        "hit_rate": (round(hits / n, 4) if n else None),',
     '        "hit_rate": round(hits / n, 4) if n else 0.0,',
     'tests/test_the_model_report_card_is_honest.py'),
    ('드리프트 기준선을 넘은 종목을 안 센다(본 적 없는 국면이 안 보인다)',
     'quant/reporting/ml_health.py',
     '            "over_line": sum(1 for r in rows if r["psi"] > PSI_NOTABLE),',
     '            "over_line": 0,',
     'tests/test_the_model_report_card_is_honest.py'),
    ('성적표에 한계를 안 싣는다(숫자만 실으면 거짓말이다)',
     'quant/reporting/ml_health.py',
     '        # 화면이 지어내지 않도록 한계도 장부가 싣는다.\n        "limits": [',
     '        "limits": [] and [',
     'tests/test_the_model_report_card_is_honest.py'),
    ('성적표가 파일로 안 나간다(계산만 맞고 화면은 빈다)',
     'quant/reporting/ml_health.py',
     '    atomic_write_json(p, report(state_dir, asof))',
     '    pass',
     'tests/test_the_model_report_card_is_honest.py'),
    ('배치가 부르는 명령이 사라진다(매일 조용히 실패한다)',
     'quant/cli.py',
     '    mlr.set_defaults(func=_cmd_ml_report)',
     '    mlr.set_defaults(func=lambda a: None)',
     'tests/test_the_model_report_card_is_honest.py'),
    ("첫 화면이 '우연과 구별되지 않는다'를 안 적는다(모르는 것이 잘하는 것처럼 읽힌다)",
     'docs/ml.html',
     '  else{verdict="실전 적중률이 <b>우연과 구별되지 않습니다</b> — 아직 잘한다는 "+',
     '  else{verdict="실전 적중률입니다. "+',
     'tests/test_the_model_report_card_is_honest.py'),
    ('확신할수록 덜 맞히는 상태를 화면이 안 알린다(가장 중요한 신호를 묻는다)',
     'docs/ml.html',
     '  var flipped=(up.actual!=null&&dn.actual!=null&&up.actual<dn.actual);',
     '  var flipped=false;',
     'tests/test_the_model_report_card_is_honest.py'),
    ('정상인 모델에도 역방향 경고를 붙인다(대조군 — 없는 결함을 지어낸다)',
     'docs/ml.html',
     "    (flipped?'<div class=\"warn\" style=\"margin-top:14px\">⚠️ <b>확신할수록 덜 '+",
     "    (true?'<div class=\"warn\" style=\"margin-top:14px\">⚠️ <b>확신할수록 덜 '+",
     'tests/test_the_model_report_card_is_honest.py'),
    ('보정표를 화면이 안 그린다(이 페이지에서 가장 중요한 표가 사라진다)',
     'docs/ml.html',
     '  var rows=(C.table||[]).filter(function(r){return r.n>0;});',
     '  var rows=[];',
     'tests/test_the_model_report_card_is_honest.py'),
    ('전 종목이 붙잡힌 상태를 화면이 말하지 않는다(자유롭게 굴리는 줄 안다)',
     'docs/ml.html',
     '    (all&&held===all?',
     '    (false?',
     'tests/test_the_model_report_card_is_honest.py'),
    ('표본 크기를 화면에서 지운다(107건과 10,000건이 같아 보인다)',
     'docs/ml.html',
     "        ' <span class=\"sub\">('+(L.hits||0)+'/'+(L.n||0)+'건)</span></div></div>'+",
     "        '</div></div>'+",
     'tests/test_the_model_report_card_is_honest.py'),
    ('성적표가 장부 목록을 직접 훑는다(보관본이 섞여 표본이 부풀고, '
     "부푼 표본은 '우연과 구별 불가'를 '엣지 입증'으로 뒤집는다)",
     'quant/reporting/ml_health.py',
     '    from quant.live.ledger_basics import ledger_files\n    return [f for f in ledger_files(state_dir)',
     '    import glob\n    return [f for f in sorted(glob.glob(os.path.join(state_dir, "paper", "*.json")))',
     'tests/test_an_archive_is_not_a_second_account.py'),
    ('상단 바에서 머신러닝 페이지를 뺀다(아무도 못 찾는다)',
     'docs/assets/nav.js',
     '    ["ml.html", "머신러닝"],',
     '',
     'tests/test_the_model_report_card_is_honest.py'),

    # ── 원화 환산이 성적을 물들이지 않는가 (2026-08-24 감사 312) ────
    ('시드를 안 바꾸고 자산만 원화로 바꾼다(환차손익이 성적에 섞인다)',
     'quant/live/krw.py',
     '        "start_cash": round(usd_base * r, 0),',
     '        "start_cash": round(usd_base, 0),',
     'tests/test_the_won_figure_is_a_reading_aid.py'),
    ('손익을 시드와 다른 환율로 낸다(하지 않은 환투자가 성적에 잡힌다)',
     'quant/live/krw.py',
     '        "pnl": round((usd_eq - usd_base) * r, 0),',
     '        "pnl": round(usd_eq * r - usd_base, 0),',
     'tests/test_the_won_figure_is_a_reading_aid.py'),
    ('환율을 모를 때 1.0으로 대신한다(1만 달러가 1만 원이 된다)',
     'quant/live/krw.py',
     '    if not (r > 0):\n        return None                     # 0이나 음수 환율은 환율이 아니다',
     '    if not (r > 0):\n        r = 1.0',
     'tests/test_the_won_figure_is_a_reading_aid.py'),
    ('아는 통화만 환산하는 관문을 연다(원화를 또 곱하거나 엔을 달러로 친다)',
     'quant/live/krw.py',
     '    if cur not in ("USD", "USDT"):\n        return None',
     '    if False:\n        return None',
     'tests/test_the_won_figure_is_a_reading_aid.py'),
    ('USDT를 달러로 친 사실을 숨긴다(깨진 적 있는 연동을 사실처럼 판다)',
     'quant/live/krw.py',
     '        "assumed_peg": (cur == "USDT"),',
     '        "assumed_peg": False,',
     'tests/test_the_won_figure_is_a_reading_aid.py'),
    ('달러 계좌에도 연동 가정을 붙인다(대조군 — 없는 한계를 지어낸다)',
     'docs/assets/trades.js',
     "      (k.assumed_peg ? ' USDT는 1달러로 가정했습니다 — 연동이 깨진 적이 ' +",
     "      (true ? ' USDT는 1달러로 가정했습니다 — 연동이 깨진 적이 ' +",
     'tests/test_the_won_figure_is_a_reading_aid.py'),
    ('뒷단계가 공개 장부에 원화를 안 붙인다(계산만 맞고 화면은 빈다)',
     'quant/reporting/krw_attach.py',
     '        d["krw"] = k',
     '        pass',
     'tests/test_the_won_figure_is_a_reading_aid.py'),
    ('뒷단계가 계좌의 자산을 원화로 덮어쓴다(감사 254 — 한 계좌에 두 통화)',
     'quant/reporting/krw_attach.py',
     '        atomic_write_json(path, d)',
     '        atomic_write_json(path, {**d, "equity": k["equity"]})',
     'tests/test_the_won_figure_is_a_reading_aid.py'),
    ('환율이 없어도 원화를 붙인다(빈칸을 지어낸 숫자로 메운다)',
     'quant/reporting/krw_attach.py',
     '        if k is None:\n            out[name] = "환율 없음 — 건너뜀"\n            continue',
     '        if k is None:\n            k = {"rate": 1.0, "equity": 0, "start_cash": 0, "pnl": 0}',
     'tests/test_the_won_figure_is_a_reading_aid.py'),
    ('장부 하나가 없으면 나머지도 포기한다(한 트랙이 전부를 막는다)',
     'quant/reporting/krw_attach.py',
     '            out[name] = "장부 없음"       # 아직 안 돈 트랙 — 정상일 수 있다\n            continue',
     '            raise',
     'tests/test_the_won_figure_is_a_reading_aid.py'),
    ('배치가 원화 환산을 안 부른다(사이트에는 영영 안 나온다)',
     '.github/workflows/guard.yml',
     '          python -m quant krw-attach --docs docs \\',
     '          true krw_attach_disabled \\',
     'tests/test_the_won_figure_is_a_reading_aid.py'),
    ('배치가 부르는 명령이 사라진다(매일 조용히 실패한다)',
     'quant/cli.py',
     '    kw.set_defaults(func=_cmd_krw_attach)',
     '    kw.set_defaults(func=lambda a: None)',
     'tests/test_the_won_figure_is_a_reading_aid.py'),
    ("화면이 '환산'이라 말하지 않는다(원화 계좌로 읽힌다 — 감사 212의 모양)",
     'docs/assets/trades.js',
     "      ' <span class=\"sub\">(원/달러 ' +",
     "      ' <span class=\"sub\" hidden>(원/달러 ' +",
     'tests/test_the_won_figure_is_a_reading_aid.py'),
    ('환율이 없는데 원화 금액을 지어낸다(빈칸을 숫자로 메운다)',
     'docs/assets/trades.js',
     '    if (!k || k.rate == null) {',
     '    if (false) {',
     'tests/test_the_won_figure_is_a_reading_aid.py'),
    ('원화 금액을 화면이 안 그린다(장부에만 있고 아무도 못 본다)',
     'docs/assets/trades.js',
     "    el.innerHTML = '한국 돈으로 보면 지금 <b>' + won(k.equity) +",
     "    el.innerHTML = '' + (0 && won(k.equity)) + (0 &&",
     'tests/test_the_won_figure_is_a_reading_aid.py'),

    # ── 배율은 기록이 벌어야 얻는다 (2026-08-25 감사 314) ───────────
    ('증명 안 된 기록에도 배율을 준다(잡음에 3배를 태운다)',
     'quant/live/leverage.py',
     '        "proven": bool(n >= MIN_ROUNDS and lo > 0.0),',
     '        "proven": True,',
     'tests/test_the_leverage_is_earned_not_chosen.py'),
    ('평균이 양수이기만 하면 증명으로 친다(신뢰구간을 안 본다 — 운 좋은 승자)',
     'quant/live/leverage.py',
     '    lo = mean - Z * se                  # 평균 수익률의 95% 하한',
     '    lo = mean',
     'tests/test_the_leverage_is_earned_not_chosen.py'),
    ('표본이 얇아도 배율을 준다(몇 회차 운으로 상한을 연다)',
     'quant/live/leverage.py',
     'MIN_ROUNDS = 120',
     'MIN_ROUNDS = 1',
     'tests/test_the_leverage_is_earned_not_chosen.py'),
    ('요동 문턱을 부동소수 잡음 크기로 되돌린다(완벽한 곡선이 상한을 가져간다)',
     'quant/live/leverage.py',
     'MIN_STD = 1e-6',
     'MIN_STD = 1e-18',
     'tests/test_the_leverage_is_earned_not_chosen.py'),
    ('위험을 안 나누고 수익만 본다(요동이 커도 같은 배율을 준다)',
     'quant/live/leverage.py',
     '        "ratio": mean / std,',
     '        "ratio": mean * 20.0,',
     'tests/test_the_leverage_is_earned_not_chosen.py'),
    ('깊은 낙폭에도 배율을 유지한다(맞고 있는데 더 크게 건다)',
     'quant/live/leverage.py',
     '    if dd >= float(drawdown_stop):',
     '    if False:',
     'tests/test_the_leverage_is_earned_not_chosen.py'),
    ('얕은 되돌림에도 1배로 내린다(대조군 — 늘 1배가 된다)',
     'quant/live/leverage.py',
     'DRAWDOWN_STOP = 0.10',
     'DRAWDOWN_STOP = 0.0001',
     'tests/test_the_leverage_is_earned_not_chosen.py'),
    ('사람이 정한 절대 천장을 넘어선다',
     'quant/live/leverage.py',
     '    reach = max(0.0, min(1.0, float(ev["ratio"]) / 0.05))',
     '    reach = max(0.0, float(ev["ratio"]) / 0.05)',
     'tests/test_the_leverage_is_earned_not_chosen.py'),
    ('선물 트랙이 옛 고정 상한을 그대로 쓴다(기록이 정한 값을 무시한다)',
     'quant/live/futures_challenger.py',
     '                             max_gross=float(lev_cap["max_leverage"]),',
     '                             max_gross=MAX_GROSS_EXPOSURE,',
     'tests/test_the_leverage_is_earned_not_chosen.py'),
    ('리포트가 지금 허락된 상한을 안 싣는다(화면이 3배라 말하는데 1배로 돈다)',
     'quant/live/futures_challenger.py',
     '        "leverage_cap": _leverage_cap(st),',
     '        "leverage_cap": None,',
     'tests/test_the_leverage_is_earned_not_chosen.py'),
    ('재다가 터지면 하드 천장으로 올라간다(모를 때 크게 건다)',
     'quant/live/futures_challenger.py',
     '        return {"max_leverage": 1.0, "proven": False,\n                "why": "배율 상한을 재지 못했습니다 — 1배로 둡니다"}',
     '        return {"max_leverage": MAX_GROSS_EXPOSURE, "proven": False,\n                "why": "배율 상한을 재지 못했습니다"}',
     'tests/test_the_leverage_is_earned_not_chosen.py'),
    ('배율 규칙이 바뀐 사실을 안 밝힌다(조용한 골대 이동)',
     'quant/live/futures_challenger.py',
     '    "on": "2026-08-25",',
     '    "on": "",',
     'tests/test_the_leverage_is_earned_not_chosen.py'),

    # ── 판정이 나면 사람 없이 적용된다 (2026-08-25 감사 315) ─────────
    ('진행 중인 실험을 패배로 친다(안 끝난 실험으로 현행을 굳힌다)',
     'quant/live/adopt.py',
     '    if s.startswith("조기 판정: 열세"):\n        return False\n    return None',
     '    return False',
     'tests/test_the_verdict_moves_the_knob_by_itself.py'),
    ('같은 판정을 매일 다시 적용한다(장부가 뒤집힌다)',
     'quant/live/adopt.py',
     '    done = {r.get("key") for r in history(state_dir)}',
     '    done = set()',
     'tests/test_the_verdict_moves_the_knob_by_itself.py'),
    ('사전 등록 안 된 비교에도 조치를 건다(사전 등록이 아니라 즉흥이 된다)',
     'quant/live/adopt.py',
     '        if not act or key in done:\n            continue',
     '        if key in done:\n            continue\n        act = act or {"knob": "alloc_method", "on_win": "erc",\n                     "on_lose": None, "why": ""}',
     'tests/test_the_verdict_moves_the_knob_by_itself.py'),
    ('되돌리기 어려운 일도 자동으로 해 버린다(실거래·원금·판매)',
     'quant/live/adopt.py',
     '        if item["manual"]:',
     '        if False:',
     'tests/test_the_verdict_moves_the_knob_by_itself.py'),
    ('보류한 판정을 기록에 안 남긴다(판정이 났는데 아무도 모른다)',
     'quant/live/adopt.py',
     '        hist.append(rec)',
     '        if rec.get("applied"):\n            hist.append(rec)',
     'tests/test_the_verdict_moves_the_knob_by_itself.py'),
    ('채택 장부가 깨지면 알 수 없는 상태로 돈다(기본값으로 안 내려간다)',
     'quant/live/adopt.py',
     '    out = dict(DEFAULTS)',
     '    out = {}',
     'tests/test_the_verdict_moves_the_knob_by_itself.py'),
    ('모르는 손잡이를 그대로 받아들인다(넘겨짚는다)',
     'quant/live/adopt.py',
     '        if knob not in DEFAULTS:\n            continue        # 모르는 손잡이는 무시한다 — 넘겨짚지 않는다',
     '        if False:\n            continue',
     'tests/test_the_verdict_moves_the_knob_by_itself.py'),
    ('본 계좌가 채택된 배분 방식을 무시한다(원장만 바뀌고 계좌는 그대로)',
     'quant/live/daily.py',
     '    prefer = str(_adopted(state_dir).get("alloc_method") or "hrp")',
     '    prefer = "hrp"',
     'tests/test_the_verdict_moves_the_knob_by_itself.py'),
    ('채택된 방식을 강제해 그날 통째로 관망이 된다(폴백이 사라진다)',
     'quant/live/daily.py',
     '    order = [prefer] + [m for m in ("hrp", "erc") if m != prefer]',
     '    order = [prefer]',
     'tests/test_the_verdict_moves_the_knob_by_itself.py'),
    ('일일 배치가 판정 적용을 안 부른다(판정이 나도 영영 안 걸린다)',
     'quant/live/daily.py',
     '        _res = apply_verdicts(state_dir,\n                              now=str(status.get("updated") or ""),\n                              status=status.get("sequential"))',
     '        _res = {"applied": [], "held": []}',
     'tests/test_the_verdict_moves_the_knob_by_itself.py'),
    ('채택 상태를 화면 재료에 안 싣는다(무엇이 채택됐는지 아무도 못 본다)',
     'quant/live/daily.py',
    ...
    ('공용 바의 대시보드 문이 딴 데로 간다(누르면 첫 화면으로 돌아온다)',
     ...
     '  var ADMIN = "index.html";',
     'tests/test_every_public_page_actually_renders.py'),
    ('공용 바의 모바일 메뉴에서 대시보드 문을 뗀다(폰에서 못 간다)',
     'docs/assets/nav.js',
     '    html.push(\'<a class="qn-mdash" href="\' + ADMIN + \'">대시보드 (운영 설정)</a>\');',
     "    html.push('');",
     'tests/test_every_public_page_actually_renders.py'),
    ('절 단위 번역을 끈다(매일 만들어지는 판단 설명이 통째로 한국어로 남는다)',
     'docs/assets/i18n.js',
     '    var parts = clauses(core);',
     '    var parts = [core];',
     'tests/test_the_site_reads_in_english_too.py'),
    ('괄호 안에서도 문장을 끊는다(신뢰구간 설명이 조각나 뜻이 깨진다)',
     'docs/assets/i18n.js',
     '      if (c === "(") { depth++; i++; continue; }',
     '      if (c === "(") { i++; continue; }',
     'tests/test_the_site_reads_in_english_too.py'),
    ('이음매를 기억하지 않고 전부 가운뎃점으로 잇는다(원문과 다른 문장이 된다)',
     'docs/assets/i18n.js',
     '    return any ? done.join("") : null;',
     '    return any ? done.join(" · ") : null;',
     'tests/test_the_site_reads_in_english_too.py'),
    ('도움말을 이스케이프한 뒤 자른다(실체 참조가 끊겨 속성이 깨진다)',
     'docs/index.html',
     '    return \'<div class="srow" title="\'+esc(String(r.reason||""))+\'">\'+',
     '    return \'<div class="srow" title="\'+esc(r.reason).slice(0,180)+\'">\'+',
     'tests/test_the_site_reads_in_english_too.py'),

    ('숫자가 든 문장 규칙을 끈다(매일 바뀌는 줄이 전부 한국어로 남는다)',
     'docs/assets/i18n.js',
     '    return (root.QUANT_EN && root.QUANT_EN.rules) || [];',
     '    return [];',
     'tests/test_the_site_reads_in_english_too.py'),
    ('규칙이 아무 문장에나 걸리게 한다(엉뚱한 영어가 숫자 옆에 붙는다)',
     'docs/assets/i18n.js',
     '        if (m) { hit = fill(rs[i][1], m, depth || 0); break; }',
     '        { hit = rs[i][1]; break; }',
     'tests/test_the_site_reads_in_english_too.py'),

    ('모르는 문장을 지어낸다(대충 맞는 영어가 숫자 옆에서 사실처럼 읽힌다)',
     'docs/assets/i18n.js',
     '    if (hit === undefined || hit === core) return null;',
     '    if (hit === undefined) hit = "(untranslated)";',
     'tests/test_the_site_reads_in_english_too.py'),
    ('한국어 화면까지 번역한다(언어를 안 골랐는데 영어가 된다)',
     'docs/assets/i18n.js',
     '    if (current() !== "en") {',
     '    if (false) {',
     'tests/test_the_site_reads_in_english_too.py'),
    ('덜 된 페이지라고 밝히지 않는다(한국어가 남은 것을 고장으로 읽는다)',
     'docs/assets/i18n.js',
     '    notice();',
     '    ;',
     'tests/test_the_site_reads_in_english_too.py'),

    # ── 판단 재료의 이름까지 옮기기($*n) · 남의 글은 그대로 두기 ──
    ('규칙이 잡아 둔 이름을 사전으로 안 보낸다(괄호 안 상태가 한국어로 남는다)',
     'docs/assets/i18n.js',
     '      if (!star || depth >= MAX_DEPTH) return g;',
     '      return g;',
     'tests/test_the_site_reads_in_english_too.py'),
    ('없는 자리를 빈칸이 아니라 글자 그대로 흘린다($3이 화면에 찍힌다)',
     'docs/assets/i18n.js',
     '      var g = m[at] === undefined ? "" : m[at];  // 안 걸린 괄호는 빈칸',
     '      var g = m[at] === undefined ? "$" + n : m[at];',
     'tests/test_the_site_reads_in_english_too.py'),
    ('모르는 이름을 지어낸다(사전에 없는 상태가 영어처럼 찍힌다)',
     'docs/assets/i18n.js',
     '      return deeper === null ? g : deeper;',
     '      return deeper === null ? "(unknown)" : deeper;',
     'tests/test_the_site_reads_in_english_too.py'),
    ('욕심 많은 규칙이 삼킨 문장을 절 단위로 다시 안 본다(가운데만 영어가 된다)',
     'docs/assets/i18n.js',
     '    if (by !== null && (hit === undefined || korean(by) < korean(hit))) {',
     '    if (by !== null && hit === undefined) {',
     'tests/test_the_site_reads_in_english_too.py'),
    ('남은 한국어를 안 센다(반쪽 영어와 온전한 영어를 구별 못 한다)',
     'docs/assets/i18n.js',
     '    return found ? found.length : 0;',
     '    return 0;',
     'tests/test_the_site_reads_in_english_too.py'),
    ('옮기지 말라고 표시한 자리도 옮긴다(커밋 제목·헤드라인이 반쪽 영어가 된다)',
     'docs/assets/i18n.js',
     '    if (node.getAttribute && node.getAttribute("data-qi18n") === "keep") return;',
     '    ;',
     'tests/test_the_site_reads_in_english_too.py'),
    ('나중에 그려진 조각은 조상을 안 보고 옮긴다(표시가 있어도 뚫린다)',
     'docs/assets/i18n.js',
     '      if (n.nodeType === 1 && n.getAttribute\n          && n.getAttribute("data-qi18n") === "keep") return true;',
     '      if (false) return true;',
     'tests/test_the_site_reads_in_english_too.py'),
    ('첫 화면에서 커밋 제목의 표시를 뗀다(규칙이 앞머리만 잡아 반쪽이 된다)',
     'docs/index.html',
     '<span data-qi18n="keep">${esc(e.title)}</span>',
     '${esc(e.title)}',
     'tests/test_the_site_reads_in_english_too.py'),
    ('브리핑 헤드라인의 표시를 뗀다(남의 기사 제목을 우리가 고쳐 적는다)',
     'docs/paper.html',
     '`<span data-qi18n="keep">${esc(b.title)}</span>`',
     '`${esc(b.title)}`',
     'tests/test_the_site_reads_in_english_too.py'),
    ('오답 노트의 판단 근거를 90자에서 자른다(잘린 꼬리는 옮길 수도 없다)',
     'docs/paper.html',
     '+esc(String(sym.reason)):"–"}</td></tr>`;',
     '+esc(String(sym.reason).slice(0,90)):"–"}</td></tr>`;',
     'tests/test_the_site_reads_in_english_too.py'),

    # ── 한국 밖에서 들어오면 영어로 맞는다 (2026-08-26 사장님 지시) ──
    ('브라우저 언어를 안 본다(영어권 방문자가 한국어 첫 화면을 만난다)',
     'docs/assets/i18n.js',
     '    return stored() || guess();',
     '    return stored() || "ko";',
     'tests/test_the_site_reads_in_english_too.py'),
    ('선호 목록 전체에서 한국어를 찾아낸다(영어를 원하는 사람에게 한국어가 나간다)',
     'docs/assets/i18n.js',
     '    return first.indexOf("ko") === 0 ? "ko" : "en";',
     '    return list.join(",").toLowerCase().indexOf("ko") >= 0 ? "ko" : "en";',
     'tests/test_the_site_reads_in_english_too.py'),
    ('언어를 못 읽으면 영어로 간다(원본이 한국어인데 한국 방문자가 영어를 본다)',
     'docs/assets/i18n.js',
     '    if (!first) return "ko";              // 모르면 원본(한국어)이다',
     '    if (!first) return "en";',
     'tests/test_the_site_reads_in_english_too.py'),
    ('짐작을 사람의 선택으로 친다(자동 전환을 밝히는 줄이 영영 안 뜬다)',
     'docs/assets/i18n.js',
     '    return /[?&]lang=(ko|en)/.test(root.location.search || "") || !!stored();',
     '    return true;',
     'tests/test_the_site_reads_in_english_too.py'),
    ('자동으로 영어가 된 것을 말하지 않는다(원문이 사라진 것처럼 보인다)',
     'docs/assets/i18n.js',
     '    guessed_note();',
     '    ;',
     'tests/test_the_site_reads_in_english_too.py'),
    ...
    # ── 방송도 영어로 나간다 (감사 325) ──────────────────────────
    ('영어 캡션이 금액 관문을 안 지난다(같은 사고가 영어로 세상에 나간다)',
     'quant/reporting/social.py',
     '    blocked = impossible_reason(x)\n    if blocked:',
     '    blocked = ""\n    if blocked:',
     'tests/test_the_caption_goes_out_in_english_too.py'),
    ('영어 짧은 판이 킬스위치·개입 고지를 버린다(쓸 말 많은 날 경고가 사라진다)',
     'quant/reporting/social.py',
     '            f"💰 {eq} (cumulative {ret}{day_line}{bench_short}){kill}{owner}\\n"',
     '            f"💰 {eq} (cumulative {ret}{day_line}{bench_short})\\n"',
     'tests/test_the_caption_goes_out_in_english_too.py'),
    ('영어 캡션이 숏을 숏이라 안 적는다(팔아 둔 종목이 산 것처럼 읽힌다)',
     'quant/reporting/social.py',
     '            name += " (short)"',
     '            pass',
     'tests/test_the_caption_goes_out_in_english_too.py'),
    ('영어 캡션의 링크가 한국어 화면으로 간다(영어로 읽고 눌렀는데 거기서 끝난다)',
     'quant/reporting/social.py',
     '        f"🔗 {site_url}?lang=en\\n"',
     '        f"🔗 {site_url}\\n"',
     'tests/test_the_caption_goes_out_in_english_too.py'),
    ('영어 캡션을 파일로 안 쓴다(어드민이 복사할 원고가 없다)',
     'quant/reporting/social.py',
     '             ("caption_instagram_en.txt", caps_en["instagram"]),',
     '',
     'tests/test_the_caption_goes_out_in_english_too.py'),
    ('포인터가 영어판을 안 싣는다(어드민 화면에는 한국어만 뜬다)',
     'quant/reporting/social.py',
     '        text = _read_if(name)',
     '        text = None',
     'tests/test_the_caption_goes_out_in_english_too.py'),
    ('어드민이 영어 캡션을 안 보여준다(만들어 두고 아무도 못 쓴다)',
     'docs/admin.html',
     '      if(d.captions.instagram_en)',
     '      if(false)',
     'tests/test_the_caption_goes_out_in_english_too.py'),

    # ── 프로그램도 영어로 읽힌다 (감사 326) ───────────────────────
    ('모르는 문장을 지어낸다(사전에 없는 말이 영어처럼 찍힌다)',
     'quant/i18n.py',
     '    return None if hit is None or hit == core else hit',
     '    return "(untranslated)" if hit is None else hit',
     'tests/test_the_program_speaks_english_too.py'),
    ('지정하지 않아도 영어로 간다(한국 사용자가 원본을 잃는다)',
     'quant/i18n.py',
     '    return v if v in _LANGS else "ko"',
     '    return v if v in _LANGS else "en"',
     'tests/test_the_program_speaks_english_too.py'),
    ('규칙이 잡아 둔 말을 사전으로 안 보낸다(값 자리의 말이 한국어로 남는다)',
     'quant/i18n.py',
     '        if not star or depth >= _MAX_DEPTH:',
     '        if True:',
     'tests/test_the_program_speaks_english_too.py'),
    ('숫자가 든 문장 규칙을 끈다(요약 표가 통째로 한국어로 남는다)',
     'quant/i18n.py',
     '    from quant.i18n_en import RULES\n    return RULES',
     '    from quant.i18n_en import RULES\n    return []',
     'tests/test_the_program_speaks_english_too.py'),
    ('명령줄 --lang 을 파서보다 늦게 읽는다(--lang en --help가 한국어로 뜬다)',
     'quant/cli.py',
     '    _pick_lang(_sys.argv[1:] if argv is None else argv)',
     '    ;',
     'tests/test_the_program_speaks_english_too.py'),
    ('도움말을 옮기지 않는다(첫 화면이 통째로 한국어로 남는다)',
     'quant/cli.py',
     '    translate_parser(parser)',
     '    ;',
     'tests/test_the_program_speaks_english_too.py'),
    ('하위 명령 설명을 옮기지 않는다(--help 목록만 한국어로 남는다)',
     'quant/i18n.py',
     '            for pseudo in getattr(act, "_choices_actions", []) or []:',
     '            for pseudo in []:',
     'tests/test_the_program_speaks_english_too.py'),
    ('조종석에 번역 엔진을 안 싣는다(프로그램 화면만 한국어 전용이 된다)',
     'quant/web/app.py',
     '            + _inline_asset("i18n-en.js") + _inline_asset("i18n.js"))',
     '            + "")',
     'tests/test_the_program_speaks_english_too.py'),
    ('조종석에서 언어 버튼을 뗀다(되돌아갈 길도 바꿀 길도 없다)',
     'quant/web/app.py',
     '            + links + _LANG_BTN + "</nav></div>")',
     '            + links + "</nav></div>")',
     'tests/test_the_program_speaks_english_too.py'),
    ('조종석이 자기 이름을 안 밝힌다(사이트 첫 화면 기준 안내가 뜬다)',
     'quant/web/app.py',
     '    return (\'<script>window.QUANT_I18N_PAGE="cockpit.html";</script>\'',
     "    return (''",
     'tests/test_the_program_speaks_english_too.py'),
    ('방송 화면에는 번역 엔진을 안 싣는다(같은 프로그램인데 한 화면만 한국어다)',
     'quant/web/app.py',
    ...
    ('운영 종목 하나의 차트 매핑을 뗀다(그 종목만 눌러도 차트가 안 뜬다)',
     'docs/assets/tv-symbols.js',
     '    QQQ: "NASDAQ",',
     '    QQQ_: "NASDAQ",',
     'tests/test_the_symbol_dialog_keeps_the_sources_apart.py'),
    ('차트가 왜 비었는지 화면이 말하지 않는다(읽는 사람은 고장으로 읽는다)',
     'docs/index.html',
     '      box.innerHTML=\'<div class="sub">이 종목은 트레이딩뷰 차트를 연결해 \'+',
     '      box.innerHTML=\'<div class="sub">\'+',
     'tests/test_the_symbol_dialog_keeps_the_sources_apart.py'),
    ('누른 종목이 아닌 차트를 띄운다(창 제목과 차트가 다른 회사가 된다)',
     'docs/index.html',
     '    var url=window.QuantTV&&QuantTV.tvEmbedUrl(key,{',
     '    var url=window.QuantTV&&QuantTV.tvEmbedUrl("crypto:BTC/USDT",{',
     'tests/test_the_first_screen_answers_the_first_question.py'),

    ('밤 배치가 정한 명단 날짜를 종목에 안 넘긴다(배선만 남고 효력이 없다)',
     'quant/live/retrain.py',
     '            out = run_retrain(market, symbol, panel_asof=roster_asof,',
     '            out = run_retrain(market, symbol,',
     'tests/test_the_panel_is_one_roster_per_night.py'),
    ('장부에 적히는 명단 날짜를 비운다(어느 명단이었는지 되살릴 수 없다)',
     'quant/live/retrain.py',
     '                                 roster_asof=roster_asof)',
     '                                 roster_asof="")',
     'tests/test_the_panel_is_one_roster_per_night.py'),
    ('회전 축을 탐색표에서 뺀다(기계가 회전을 고를 길이 사라진다)',
     'quant/live/retrain.py',
     '    "band_mult": _axis_band_mult,',
     '    "band_mult_": _axis_band_mult,',
     'tests/test_the_machine_picks_its_own_turnover.py'),
    ('선발전에서 도전자를 챔피언 밴드로 돌린다(회전 축이 아무것도 못 잰다)',
     'quant/live/champion_challenger.py',
     '        res_h = Backtester(self.challenger, **bt_h).run(df)',
     '        res_h = Backtester(self.challenger, **bt).run(df)',
     'tests/test_the_machine_picks_its_own_turnover.py'),
    ('홀드아웃이 스펙별 밴드 배수를 무시한다(동시검정·패널에서 회전 축이 죽는다)',
     'quant/live/retrain.py',
     '            r_ch = Backtester(build(spec), **_kw(spec)).run(df).returns',
     '            r_ch = Backtester(build(spec), **bt_kwargs).run(df).returns',
     'tests/test_the_machine_picks_its_own_turnover.py'),
    ('체결기가 챔피언의 밴드 배수를 안 따른다(기계가 고른 회전이 링 안에서만 산다)',
     'quant/live/daily.py',
     '    return base * mult',
     '    return base',
     'tests/test_the_machine_picks_its_own_turnover.py'),
    ('패널 관문을 승격에서 뗀다(한 종목에서만 좋아 보이는 설정이 그대로 승격된다)',
     'quant/live/retrain.py',
     '        if panel_res and panel_res.get("blocked"):',
     '        if False:',
     'tests/test_the_panel_gate_decides_promotion.py'),
    ('패널 판정이 없을 때도 막는다(변이 후보가 영영 승격 못 해 언덕오르기가 죽는다)',
     'quant/live/retrain.py',
     '        if panel_res and panel_res.get("blocked"):\n            return _out(',
     '        if not panel_res or panel_res.get("blocked"):\n            return _out(',
     'tests/test_the_panel_gate_decides_promotion.py'),
    ('못 잰 판정을 위반으로 읽는다(종목 적게 돈 밤마다 승격이 얼어붙는다)',
     'quant/live/retrain.py',
     '                if rec.get("spec_key") != key or rec.get("skipped"):',
     '                if rec.get("spec_key") != key:',
     'tests/test_the_panel_gate_decides_promotion.py'),
    ('낡은 패널 판정도 그대로 쓴다(몇 달 전 판정이 오늘 승격을 막는다)',
     'quant/live/retrain.py',
     '            if age >= max(1, int(max_age_nights)):',
     '            if False:',
     'tests/test_the_panel_gate_decides_promotion.py'),
    ('밤 기록에서 패널 판정 칸을 뺀다(관문의 효과를 나중에 잴 수 없다)',
     'quant/live/retrain.py',
     '        "panel_gate": decision.get("panel_gate"),',
     '        "panel_gate": None,',
     'tests/test_the_panel_gate_decides_promotion.py'),
    ('선물 체결기의 밴드를 끈다(같은 챔피언이 트랙마다 다른 회전으로 돈다)',
     'quant/live/futures_challenger.py',
     '            if abs(delta) / slice_budget < band:',
     '            if abs(delta) / slice_budget < 0.0:',
     'tests/test_the_futures_track_picks_its_own_turnover.py'),
    ('선물 밴드가 청산까지 막는다(관망 신호의 잔여 포지션이 영영 남는다)',
     'quant/live/futures_challenger.py',
     '        if band > 0 and target != 0 and slice_budget > 0:',
     '        if band > 0 and slice_budget > 0:',
     'tests/test_the_futures_track_picks_its_own_turnover.py'),
    # ── 2026-09-05 · 합성된 화면에서도 영어가 끝까지 간다 ─────────────────
    ('buy_hold 해설을 절 목록이 아니라 한 덩어리로 되돌린다(합성된 화면에서 한국어가 남는다)',
     'quant/live/explain.py',
     '        return f"{head} — " + " — ".join(_WHY_BUY_HOLD_CLAUSES)',
     '        return f"{head} — " + "".join(_WHY_BUY_HOLD_CLAUSES)',
     'tests/test_the_site_reads_in_english_too.py'),
    # ── 2026-09-07 · 패널 관문이 다중검정 값을 치른다 ──────────────────────
    ('패널 관문이 동시검정을 안 본다(t 하나로만 판정 — 보정이 통째로 빠진다)',
     'quant/live/retrain.py',
     '                return {"blocked": (not passed) or bool(rc_block),',
     '                return {"blocked": (not passed),',
     'tests/test_the_panel_gate_pays_for_its_multiple_testing.py'),
    ('동시검정을 재려다 실패해도 승격시킨다(관문을 못 건 채로 통과한다)',
     'quant/live/retrain.py',
     '                    if rc.get("broken"):',
     '                    if False:',
     'tests/test_the_panel_gate_pays_for_its_multiple_testing.py'),
    ('밤 단위 읽기가 동시검정을 안 들고 온다(관문이 볼 방법이 없어진다)',
     'quant/live/retrain.py',
     '            "reality_check": _night_reality_check(recs),',
     '            "reality_check": {},',
     'tests/test_the_panel_gate_pays_for_its_multiple_testing.py'),
    ('한 밤의 회차 중 좋은 쪽 동시검정을 고른다(보수적이지 않은 값이 관문을 연다)',
     'quant/live/retrain.py',
     '            "p": max(float(rc.get("p") or 0.0) for rc in measured),',
     '            "p": min(float(rc.get("p") or 0.0) for rc in measured),',
     'tests/test_the_panel_gate_pays_for_its_multiple_testing.py'),
    ...
    ('ETF에도 증권거래세를 물린다(안 내는 세금을 왕복 15bp 물고, 그 값이 밴드·오디션·손익으로 다 흘러간다)',
     ...
     '    "kr_stock_etf": {"fee": 0.00015 + 0.00075, "slippage": 0.0005,',
     'tests/test_an_etf_does_not_pay_the_stock_tax.py'),
    ('ETF 프리셋을 고르지 않는다(표에만 있고 아무도 안 읽는 값이 된다)',
     'quant/backtest/costs.py',
     '        if overrides.get("is_etf") and f"{key}_etf" in MARKET_COST_PRESETS:',
     '        if False:',
     'tests/test_an_etf_does_not_pay_the_stock_tax.py'),
    ('ETF 프리셋으로 갈아타며 시장을 잃는다(호가 단위 하한이 조용히 0이 된다)',
     'quant/backtest/costs.py',
     '        base.setdefault("market", market)      # 틱 하한이 시장을 알아야 한다',
     '        base.setdefault("market", "")',
     'tests/test_an_etf_does_not_pay_the_stock_tax.py'),
    ('체결 비용이 종목을 안 본다(교정이 표에만 남고 장부는 그대로 틀린다)',
     'quant/live/daily.py',
     '    cm = CostModel.for_market(market, is_etf=is_etf(market, symbol))',
     '    cm = CostModel.for_market(market)',
     'tests/test_an_etf_does_not_pay_the_stock_tax.py'),
    ('밴드가 종목을 안 본다(비용 교정이 회전 통제까지 안 닿는다)',
     'quant/live/daily.py',
     '    base = _rebalance_band_rel(market, state_dir, _s)',
     '    base = _rebalance_band_rel(market, state_dir)',
     'tests/test_an_etf_does_not_pay_the_stock_tax.py'),
    ('공개 비용 기준이 한국을 한 숫자로만 말한다(ETF와 주식 중 하나는 반드시 거짓말이 된다)',
     'quant/live/daily.py',
     '            "kr_stock", state_dir, symbol="069500.KS").total_one_way()) * 1e4, 1)',
     '            "kr_stock", state_dir).total_one_way()) * 1e4, 1)',
     'tests/test_an_etf_does_not_pay_the_stock_tax.py'),

    # ── 2026-09-03 · 방향 관문 — 내림에 거는 것을 기계가 판정한다 ────────
    ('선물이 방향 판정을 안 물어본다(관문이 장식이 되고 매매는 그대로다)',
     'quant/live/futures_challenger.py',
     '                sym, state_dir, allow_two_sided=two_sided_ok)',
     '                sym, state_dir, allow_two_sided=True)',
     'tests/test_the_machine_judges_whether_to_bet_on_falls.py'),
    ('판정이 닫혀도 양방향으로 세운다(끄기로 한 숏이 그대로 나간다)',
     'quant/live/futures_challenger.py',
     '    if not can_short(spec) or not allow_two_sided:',
     '    if not can_short(spec):',
     'tests/test_the_machine_judges_whether_to_bet_on_falls.py'),
    ('판정이 없어도 막는다(못 잰 것을 위반으로 세면 첫날에 실험이 얼어붙는다)',
     'quant/live/direction_gate.py',
     '    if not v:\n        return True, None',
     '    if not v:\n        return False, None',
     'tests/test_the_machine_judges_whether_to_bet_on_falls.py'),
    ('낡은 판정으로 오늘을 막는다(그 사이 챔피언이 바뀐 종목에 근거 없는 제약)',
     'quant/live/direction_gate.py',
     '        if age < 0 or age >= max_age_nights:',
     '        if age < 0:',
     'tests/test_the_machine_judges_whether_to_bet_on_falls.py'),
    ('방향 문턱을 따로 정한다(관문이 아니라 편의가 된다)',
     'quant/live/direction_gate.py',
     '    return float(PANEL_T_REF)',
     '    return 0.0',
     'tests/test_the_machine_judges_whether_to_bet_on_falls.py'),
    ('숏을 못 내는 챔피언을 차이 0으로 담는다(없는 관측이 패널 평균을 끈다)',
     'quant/live/retrain.py',
     '            if dseries is not None and len(dseries):',
     '            if dseries is not None:',
     'tests/test_the_machine_judges_whether_to_bet_on_falls.py'),
    ('밤 배치가 방향 재료를 안 줍는다(관문이 영원히 재료 부족으로 침묵한다)',
     'quant/live/retrain.py',
     '                               direction_probe=True)',
     '                               direction_probe=False)',
     'tests/test_the_machine_judges_whether_to_bet_on_falls.py'),
    ('선물 밴드 배수를 1로 못 박는다(밤 오디션이 고른 회전을 안 따른다)',
     'quant/live/futures_challenger.py',
     '        return base * band_mult_of(_spec(symbol, state_dir).get("params"))',
     '        return base',
     'tests/test_the_futures_track_picks_its_own_turnover.py'),
    ('수수료 예산이 노출 확대를 안 막는다(폭주가 다시 와도 그냥 커진다)',
     'quant/live/futures_challenger.py',
     '        if grow > 0 and not allow_growth:',
     '        if grow > 0 and False:',
     'tests/test_the_futures_track_picks_its_own_turnover.py'),
    ('수수료 예산이 줄이는 거래까지 막는다(안전장치가 그 자체로 위험이 된다)',
     'quant/live/futures_challenger.py',
     '        if grow > 0 and not allow_growth:\n            continue',
     '        if not allow_growth:\n            continue',
     'tests/test_the_futures_track_picks_its_own_turnover.py'),
    ('예산을 체결 **뒤에** 잰다(이번 회차 수수료가 섞여 한 회차 늦게 잡힌다)',
     'quant/live/futures_challenger.py',
     '    budget = fee_budget(st)',
     '    budget = {}',
     'tests/test_the_futures_track_picks_its_own_turnover.py'),
    ('그림자 계좌가 시장 무관 고정 수수료로 되돌아간다(본 계좌와 자가 다르다)',
     'quant/live/gen2.py',
    ...
    ('밤 검증이 비용 없이 돈다(검증 성적이 수수료 빼기 전으로 돌아간다)',
     'quant/cli.py',
     '    fee_kw = {"fee": fee_one_way} if fee_one_way is not None else {}',
     '    fee_kw = {}',
     'tests/test_validation_is_measured_after_fees.py'),
    ('배치 건강 기록을 UTC 날짜로 묶는다(자정 넘긴 2회차가 1회차를 덮어쓴다)',
     'quant/live/daily.py',
     '        today = night_key()',
     '        today = _date.today().isoformat()',
     'tests/test_the_batch_health_is_keyed_by_the_korean_night.py'),
    ('선물 보고서에서 수수료 빼기 전 성적을 뺀다(전략이 잃은 것처럼 읽힌다)',
     'quant/live/futures_challenger.py',
     '        "gross_return_pct": _gross_return_pct(st, eq),',
     '        "gross_return_pct": None,',
     'tests/test_the_futures_page_shows_the_strategy_before_fees.py'),
    ('선물 페이지가 수수료 빼기 전 칸을 안 그린다(순수익률만 남는다)',
     'docs/futures.html',
     "      (d.gross_return_pct!=null?",
     "      (false?",
     'tests/test_the_futures_page_shows_the_strategy_before_fees.py'),
    ('배치 장부에 명단을 안 넘긴다(못 돈 종목이 어느 칸에도 안 남는다)',
     'quant/live/retrain.py',
     '                      roster=[_key(m, s) for m, s in targets])',
     '                      roster=None)',
     'tests/test_the_batch_says_what_it_never_reached.py'),
    ('못 돈 종목을 장부에서 뺀다(40 중 24만 돌아도 산술이 닫힌 것처럼 보인다)',
     'quant/live/daily.py',
     '        entry["not_reached"] = len(missing)',
     '        entry["not_reached"] = 0',
     'tests/test_the_batch_says_what_it_never_reached.py'),
    ('못 돈 밤을 전 종목 실패로 올린다(실패가 0인데 전멸이라고 말한다)',
     'quant/live/retrain.py',
     '    if targets and not ok and not skipped and not not_reached:',
     '    if targets and not ok and not skipped:',
     'tests/test_the_batch_says_what_it_never_reached.py'),
    ('밤 열쇠를 화면 목록에 안 싣는다(카드·경보가 늘 옛 경로로 되돌아간다)',
     'quant/live/daily.py',
     '                "night": rec.get("night"),',
     '                "night": None,',
     'tests/test_the_public_card_counts_the_night_not_the_bar_date.py'),
    ('공개 카드가 그날 오디션을 봉 날짜로 고른다(후보 수가 5분의 1이 된다)',
     'docs/sns_card.html',
     '    return (r.night||r.asof)===date});',
     '    return (r.asof)===date});',
     'tests/test_the_public_card_counts_the_night_not_the_bar_date.py'),
    ('공회전 경보가 밤을 봉 날짜로 고른다(주식 쪽 공회전이 통째로 빠진다)',
     'quant/live/flag_watch.py',
     '        day = max(_night(r) for r in vac)',
     '        day = max((r.get("asof") or "") for r in vac)',
     'tests/test_the_public_card_counts_the_night_not_the_bar_date.py'),
    ('재학습 기록의 밤 열쇠를 봉 날짜로 바꾼다(밤이 쪼개지고 두 밤이 붙는다)',
     'quant/live/retrain.py',
     '        "night": str(panel_asof or night_key()),',
     '        "night": str(asof),',
     'tests/test_the_banner_counts_the_night_not_the_bar_date.py'),
    ('첫 화면 배너가 밤을 봉 날짜로 센다(어젯밤 24종목이 코인 5개가 된다)',
     'quant/web/app.py',
     '                return str(rec.get("night") or rec.get("asof") or "")',
     '                return str(rec.get("asof") or "")',
     'tests/test_the_banner_counts_the_night_not_the_bar_date.py'),
    ('밤의 두 회차를 합칠 재료를 장부에 안 남긴다(그 밤은 영영 못 합친다)',
     'quant/live/retrain.py',
     '            **_panel_terms(collector, v["spec_key"]),',
     '            **{},',
     'tests/test_the_two_runs_of_a_night_can_be_added_up.py'),
    ('장부를 봉 날짜로 묶는다(한 밤이 쪼개지고 두 밤이 한 칸에 붙는다)',
     'quant/live/retrain.py',
     '        key = str(rec.get("roster_asof") or rec.get("asof") or "")',
     '        key = str(rec.get("asof") or "")',
     'tests/test_the_two_runs_of_a_night_can_be_added_up.py'),
    ('재료 없어 못 합친 설정을 조용히 뺀다("그 밤엔 없었다"와 구별이 안 된다)',
     'quant/live/retrain.py',
     '                    unmergeable.append(key)',
     '                    pass',
     'tests/test_the_two_runs_of_a_night_can_be_added_up.py'),
    ('회차를 포갤 때 앞 회차 값을 덮어쓴다(합쳐도 마지막 회차만 남는다)',
     'quant/live/panel_gate.py',
     '            sums[d] = sums.get(d, 0.0) + float(s)',
     '            sums[d] = float(s)',
     'tests/test_the_two_runs_of_a_night_can_be_added_up.py'),
    ('겹친 종목을 못 본다(같은 종목이 두 표를 행사해 패널 평균이 기운다)',
     'quant/live/panel_gate.py',
     '        overlap |= (seen & syms)',
     '        overlap |= set()',
     'tests/test_the_two_runs_of_a_night_can_be_added_up.py'),
    ('합친 뒤 종목 모자란 날을 안 버린다(두 종목짜리 날이 한 표를 갖는다)',
     'quant/live/panel_gate.py',
     '            if int(c) >= min_symbols]',
     '            if int(c) >= 0]',
     'tests/test_the_two_runs_of_a_night_can_be_added_up.py'),
    ('쪼개진 밤을 장부가 표시하지 않는다(측정 고장이 성적 부진처럼 보인다)',
     'quant/live/retrain.py',
     '    if len(verdicts) > PANEL_ROSTER_PER_NIGHT:',
     '    if False and len(verdicts) > PANEL_ROSTER_PER_NIGHT:',
     'tests/test_the_panel_is_one_roster_per_night.py'),
    ('재운 후보를 장부에서 지운다(0이 정상인지 고장인지 구별할 수 없어진다)',
     'quant/live/retrain.py',
     '        "asleep_candidates": asleep,',
     '        "asleep_candidates_": asleep,',
     'tests/test_the_sleeping_guard_actually_wakes_up.py'),
    ('횡단면 증거를 다시 하루치로 되돌린다(시장 하나가 통째로 빠진다)',
     'quant/live/crosssection.py',
     '        paths = sorted(latest_per_symbol(state_dir).values())',
     '        paths = sorted(glob.glob(os.path.join(\n'
     '            _fullest_snapshot(state_dir) or "", "*.csv.gz")))',
     'tests/test_the_same_setting_is_judged_across_symbols.py'),
    ('여러 밤에서 모았다는 사실을 문장에서 지운다(하루치로 읽힌다)',
     'quant/live/crosssection.py',
     '    when = (f"{ev.get(\'asof\', \'?\')} 스냅샷" if len(days) <= 1 else',
     '    when = (f"{ev.get(\'asof\', \'?\')} 스냅샷" if True else',
     'tests/test_the_same_setting_is_judged_across_symbols.py'),
    ('같은 종목의 옛 밤 스냅샷도 함께 센다(표본이 근거 없이 부푼다)',
     'quant/live/crosssection.py',
     '            picked.setdefault(f"{parsed[0]}:{parsed[1]}", path)',
     '            picked[f"{parsed[0]}:{parsed[1]}"] = path',
     'tests/test_the_same_setting_is_judged_across_symbols.py'),
    ('이동평균 하락 문장의 영어 규칙을 짐작대로 되돌린다(한 번도 안 맞던 그 상태)',
     'docs/assets/i18n-en.js',
     '      ["^이동평균 교차: (\\\\d+)일선이 (\\\\d+)일선 아래 \\\\(하락\\\\/횡보 추세 판단\\\\)$",',
     '      ["^이동평균 교차: (\\\\d+)일선이 (\\\\d+)일선 아래 \\\\(하락 추세 판단\\\\)$",',
     'tests/test_the_site_reads_in_english_too.py'),
    ('새 전략 이름이 든 문장의 영어 규칙을 챔피언 전용으로 좁힌다(새 전략 승격일에 한국어가 뜬다)',
     'docs/assets/i18n-en.js',
     '      ["^(.+) 전략 신호에 따름$", "following the $1 strategy\'s signal"],',
     '      ["^(챔피언) 전략 신호에 따름$", "following the $1 strategy\'s signal"],',
     'tests/test_the_site_reads_in_english_too.py'),
    ('명단 날짜를 UTC 달력일로 되돌린다(밤의 두 회차가 자정에 갈린다)',
     'quant/live/retrain.py',
     '    roster_asof = str(kwargs.pop("panel_asof", "") or night_key())',
     '    roster_asof = str(kwargs.pop("panel_asof", "") or _today_iso())',
     'tests/test_the_panel_is_one_roster_per_night.py'),
    ('밤 열쇠의 시간대를 UTC로 바꾼다(한국 자정 대신 UTC 자정이 경계가 된다)',
     'quant/live/retrain.py',
    ...
    ('장부에서 종목별 판정 칸을 지운다(대조할 재료가 안 쌓인다)',
     ...
     'tests/test_the_panel_gate_measures_the_setting_not_the_symbol.py'),
    # ── 그림자 장부는 한 계좌만 담는다 (2026-09-07) ──────────────────
    # 근본 원인은 시계가 아니라 **재료를 두 계좌가 줬다**는 것이다.
    ("섀도 대조군 회차도 그림자 실험에 재료를 준다(두 계좌가 한 장부에 쓴다)",
     "quant/live/daily.py",
     "    if use_champions:\n        try:\n            from quant.live.alloc_ladder import run_alloc_ladder",
     "    if True:\n        try:\n            from quant.live.alloc_ladder import run_alloc_ladder",
     "tests/test_the_shadow_ledgers_hold_one_account.py"),
    ("화면이 마지막으로 **쓴** 줄을 읽는다(더 과거의 봉을 오늘로 발표한다)",
     "quant/live/shadow_clock.py",
     "        if best is None or _day(row) >= _day(best):\n            best = row",
     "        best = row",
     "tests/test_the_shadow_ledgers_hold_one_account.py"),
    ("표본 일수를 줄 수로 센다(같은 날짜를 두 번 세어 표본을 부풀린다)",
     "quant/live/shadow_clock.py",
     "    return len({_day(r) for r in history or () if _day(r)})",
     "    return len([r for r in history or () if _day(r)])",
     "tests/test_the_shadow_ledgers_hold_one_account.py"),
    ("섞여 든 줄을 조용히 넘긴다(원래 그런 곡선이었다와 구별이 안 된다)",
     "quant/live/shadow_clock.py",
