...
def test_the_card_says_why_it_is_a_day_behind(_views):
    """왜 하루 늦는지를 화면이 말한다 (감사 306).

    사장님 질문(2026-08-22): *"코인은 계속 실시간으로 결과가 나오는데
    100만원 챌린지는 1일이나 지나서 나오네?"*

    고장이 아니다. 이 계좌는 **일봉**으로 판단한다 — 하루가 끝나야 그날
    봉이 닫히고, **닫힌 봉으로만** 판단한다(아직 만들어지는 중인 봉으로
    판단하면 같은 날을 다시 계산할 때 다른 답이 나온다. 이 저장소가 가장
    엄격하게 지키는 재현성이 거기서 깨진다). 그래서 새벽 배치가 확정하는
    것은 전날까지의 값이다.

    코인 단타는 1시간봉을 15분마다 보므로 하루 종일 움직인다. 둘 다
    정상이고 다른 것은 판단 주기다.

    설명 없이 날짜만 있으면 읽는 사람은 그 하루를 **고장으로** 읽는다 —
    사장님이 그러셨듯이. 맞는 질문이었고, 답이 화면에 없던 것이다.
    """
    dated = [t for t in _views["라벨"] if "확정" in t]
    assert dated, f"확정 라벨 자체가 없다: {_views['라벨']}"
    assert any("일봉" in t for t in dated), (
        "확정 자산 카드가 '왜 하루 늦는지'를 말하지 않는다 — 설명 없는 "
        f"날짜는 고장으로 읽힌다: {dated}")
