...
def test_the_recorded_weight_is_the_ordered_weight():
    ...
    # 시가 체결 기록 한 곳만 정확히 집는다 — 느슨하게 찾으면 엉뚱한
    # `"weight": round(...)`(예약 대기열 쪽)에 걸려 검사가 헛돈다.
    rec = re.search(r'fills\.append\(\{"key": key, "price": round\(fopen, 6\),'
                    r' "bar": fbar,\s*\n\s*"weight": round\((.+?), 4\),'
                    r'\s*\n\s*"type": "시가"', src)
    assert rec, "시가 체결 기록을 찾지 못했다 — 검사가 낡았다"
    recorded = rec.group(1).strip()

    assert recorded == ordered, (
