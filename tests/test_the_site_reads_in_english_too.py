...
■ 사전이 낡는 방식 — 그리고 그것을 어떻게 다루기로 했나 (2026-08-26)

이 사전은 한국어 문장 **전체**를 열쇠로 쓴다. 그래서 **사전을 쓰던 순간
화면에 있던 것만** 덮는다. 화면이 다른 상태가 되면 그때마다 영어가 조금씩
무너진다. 하루에 세 축에서 터진 날이 있었다(2026-08-26):

  · 장부 날짜가 바뀌었을 때 — 열쇠에 그날 날짜가 박혀 있었다
  · 계좌가 **숏**으로 돌아섰을 때 — 롱 라벨만 사전에 있었다
  · 코인 단타가 **전량 현금**이 됐을 때 — 보유가 있을 때 문구만 있었다

앞의 둘은 이제 검사가 **미리** 잡는다(날짜 자리 목록 / 삼항 라벨 짝). 세
번째(계좌가 어떤 상태가 될지)는 정적으로 알 수 없어 실행해 봐야 안다.

**사장님 결정(2026-08-26): 지금처럼 유지한다(A안).** 새 상태가 나오면 이
검사가 빨간불로 알려주고, 그때 문구를 채운다. 검사를 '고정 문구만'으로
좁혀 조용하게 만드는 쪽(B안)은 택하지 않았다 — 그러면 영어 구멍을 늦게
알게 되고, 이 저장소는 "화면이 조용히 틀린 것"을 가장 싫어한다.

⚠️ 그러니 이 파일이 미번역 문구로 빨개지는 것은 **고장이 아니라 발견**이다.
   검사를 무르게 고치지 말고 사전을 채울 것. 남은 상태가 많지 않아 며칠이면
   대부분 채워진다.
"""
...
# ── 종목 이름은 사전이 **자라는 목록을 따라가야** 한다 ──────────
#
# 2026-08-26에 세 번째로 같은 병을 만났다. 뿌리는 하나다:
# **목록은 규칙으로 자라는데, 그 목록을 받아 적는 표는 손으로 관리된다.**
#
#   ① 트레이딩뷰 거래소 표 — 유니버스가 20 → 42로 자랐는데 표는 20에서
#      멈춰, 잔고 1위(UUP)를 눌러도 차트가 안 떴다 (2026-08-25).
#   ② SYMBOL_INFO 이름 — 같은 확장에서 스물두 종목이 이름 없이 생
#      티커로 화면에 나왔다 (2026-08-26 오전).
#   ③ **영어 사전** — 여기. 이름 마흔둘 중 스물하나가 영어로 옮겨져
#      있지 않았다. 영어 화면에서 그 종목들은 한국어로 남았다.
#
# ③은 ①②보다 나빴다. 그냥 안 옮겨진 것이 아니라 **틀린 문장이 찍혔다**:
# 사전은 문장을 <b> 단위로 잘린 **조각**으로도 담는데, 잔고 다리 문장에서
# 잘린 조각 하나가 하필 조사 "은" 한 글자였다. 그리고 SLV의 이름이
# 바로 "은"(silver)이다. 영어 화면에서 은 보유의 **이름 자리에**
# "in this table is measured"가 찍혔다 — 금액 바로 옆에서.
#
# 조용히 틀린 화면은 이 저장소가 가장 싫어하는 것이다. 그래서 두 방향으로
# 막는다: 이름이 사전에 **있는가**(아래 첫 검사), 그리고 이름 자리를
# **문장 조각이 차지하지 않았는가**(둘째 검사).


def _symbol_names() -> list[str]:
    """운용 중인 종목의 **한국어 이름**들 — quant/markets.py가 원본이다."""
    src = (ROOT / "quant" / "markets.py").read_text("utf-8")
    names = re.findall(r'"name"\s*:\s*"([^"]+)"', src)
    return sorted({n for n in names if re.search(r"[가-힣]", n)})


def _dict_keys() -> set[str]:
    return set(re.findall(r'^\s*"([^"]+)"\s*:\s*"', DICT, re.M))


def test_every_symbol_name_can_be_read_in_english():
    """종목 이름은 화면에서 가장 눈에 띄는 한국어다 — 표의 첫 칸이다.

    이 검사는 **실행해 보지 않고** 잡는다. 브라우저 검사는 그날 장부가
    들고 있는 종목만 그리므로, 안 들고 있는 종목의 구멍은 못 본다.
    목록(markets.py)과 사전을 직접 맞대야 마흔둘을 다 본다.
    """
    missing = [n for n in _symbol_names() if n not in _dict_keys()]
    assert not missing, (
        f"영어 사전에 종목 이름이 없다({len(missing)}건) — 영어 화면에서 "
        f"이 종목들은 한국어로 남는다: {missing}")


# 이름 자리에 와도 되는 값의 꼴 — **이름**이지 문장이 아니다.
# ("Silver", "US long-term Treasuries", "Gold (Korea-listed)" … 는 되고,
#  "in this table is measured" 처럼 소문자로 시작하는 절은 안 된다.)
_NAME_LIKE = re.compile(r"^[A-Z0-9&][^.!?]*$")


def test_no_sentence_fragment_squats_on_a_symbol_name():
    """대조군 — 이름이 사전에 **있기만** 해서는 부족하다.

    ⚠️ 실제로 일어난 일이다(2026-08-26). "은"이 사전에 있긴 했다. 다만
       종목 이름이 아니라 **잘린 문장 조각**의 열쇠로 있었다. 위 검사만
       있었다면 초록이었을 것이고, 화면은 계속 틀린 채였을 것이다.
       있는가와 맞는가는 다른 질문이다.
    """
    keys = dict(re.findall(r'^\s*"([^"]+)"\s*:\s*"([^"]*)"', DICT, re.M))
    bad = {n: keys[n] for n in _symbol_names()
           if n in keys and not _NAME_LIKE.match(keys[n])}
    assert not bad, (
        "종목 이름 자리를 문장 조각이 차지했다 — 영어 화면에서 종목 "
        f"이름 대신 저 문장이 금액 옆에 찍힌다: {bad}")


def test_no_symbol_is_named_after_a_bare_particle():
    """반대쪽에서 잠근다 — 조사와 **같은 이름**의 종목이 없어야 한다.

    사전에는 "와"·"과"·"에"처럼 조사 한 글자짜리 열쇠가 아직 남아 있다
    (링크·굵은 글씨 사이에서 잘린 조각들이다). 그것들이 안전한 이유는
    단 하나 — **자산 이름이 될 수 없는 글자**여서다. 그 전제가 깨지면
    "은"에서 났던 일이 그대로 다시 난다.

    ("은"과 "금"은 이제 조각이 아니라 종목 이름으로만 사전에 있다.)
    """
    particles = {"는", "이", "가", "을", "를", "와", "과", "에", "의",
                 "로", "도", "만", "께", "부터", "까지"}
    clash = [n for n in _symbol_names() if n in particles]
    assert not clash, (
        f"조사와 이름이 같은 종목이 생겼다: {clash} — 사전에서 그 글자가 "
        "문장 조각으로도 쓰이는지 확인하고, 쓰인다면 그 문장을 먼저 고칠 것")


def test_the_balance_bridge_no_longer_breaks_into_particles():
    """잔고 다리 문장이 **조사로 끝나는 조각**을 만들지 않는다.

    이 문장은 숫자마다 <b>로 잘려 사전에 조각으로 들어간다. 조각이
    "은" 한 글자가 되면 그 열쇠가 종목 이름과 충돌한다 — 실제로 그랬다.
    조사 앞에 낱말을 하나 두면(", 기준은") 조각이 스스로 뜻을 갖는다.
    """
    assert "'이 표의 손익 <b>'" in IDX, "잔고 다리 문장이 되돌아갔다"
    assert "'</b>, 기준은 '" in IDX or "\'</b>, 기준은 \'" in IDX or (
        "</b>, 기준은" in IDX), "조사 앞 낱말이 사라졌다"
    assert "'</b>은 '" not in IDX, (
        "잔고 다리가 다시 조사 한 글자를 조각으로 만든다")
