...
def test_a_normal_over_budget_line_survives(tmp_path):
    ...
    ⚠️ 이 줄을 **자세히 보기에서** 봐야 하는 이유가 하나 더 있다(감사 278).
       위의 간단 보기 검사는 🚨가 아닌 경고를 아예 못 본다 — 접혀 있으면
       `inner_text()`에 안 들어오기 때문이다. 그래서 "못 믿을 종목을 예산
       줄에서 빼는" 안전장치를 통째로 없애도 간단 보기 검사는 조용했다.
       야간 변이 전수가 그 한 건을 끝까지 살려서 알려 줬다.
       접힌 화면은 없는 화면이 아니다 — 펴면 사람이 읽는다.
    """
    txt = _render(tmp_path, "index.html", "#side-flags", detail=True)
    assert "이더리움" in txt and "80,582원" in txt, (
        f"정상 기록까지 함께 지웠다:\n{txt[:600]}")

    line = next((l for l in txt.splitlines() if "예산을 끌어 쓴 종목" in l), None)
    assert line, f"예산 경고 줄을 못 찾았다 — 검사가 낡았다:\n{txt[:600]}"
    # 못 믿을 종목(비앤비 4,526,594.72원 — 그날 자산 997,198원)은 이 줄에
    # 있으면 안 된다. `won()`이 반올림하므로 두 표기를 모두 막는다.
    assert "비앤비" not in line, (
        f"못 믿을 종목이 '예산을 끌어 쓴' 줄에 살아 있다:\n{line}")
    assert "4,526,595" not in line and "4,526,594" not in line, (
        f"계좌보다 큰 금액을 예산 줄이 그대로 읽어 준다:\n{line}")
