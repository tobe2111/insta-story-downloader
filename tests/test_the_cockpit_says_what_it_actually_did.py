...
def _text(html_str: str) -> str:
    """화면에 **보이는 글자**만. 스크립트 본문은 화면이 아니다.

    ⚠️ 2026-09-08 CI가 잡은 자리다. 조종석은 정적 경로가 없는 단일 파일이라
       영어 사전(i18n-en.js)을 페이지에 **통째로 인라인**한다. 태그만 지우면
       그 사전의 한국어 산문이 '화면 글자'에 섞여 들어온다 — 그날 사전에
       "…조정했습니다"로 끝나는 경보 문구가 들어오자, 조종석이 그런 말을
       한 적이 없는데 아래 대조군 검사가 걸렸다.

       스크립트 본문을 먼저 버린다. 이 검사들이 묻는 것은 **조종석이 무슨
       말을 했나**이지 페이지에 어떤 코드가 실려 있나가 아니다.
    """
    body = re.sub(r"<script\b[^>]*>.*?</script>", " ", html_str,
                  flags=re.S | re.I)
