...
def test_the_real_ledger_still_contains_the_contradiction():
    ...
    # ⚠️ 여기 한 줄이 더 있었다: `"us_stock:AMZN" not in d["positions"]`.
    #    "그날 체결됐다고 적힌 것이 실제 잔고로 이어지지 않았다"를 확인하려던
    #    것인데, 읽은 값이 **오늘의 잔고**였다. 2026-08-20에 미국주식 종목이
    #    늘면서 계좌가 아마존을 **정상적으로** 매수하자 그 줄이 빨간불이
    #    됐고, 이 파일이 지키던 변이 3건이 전부 "검사 자체 고장"으로 찍혔다
    #    — **정상적인 매수 하나가 안전장치 셋을 조용히 껐다.**
    #
    #    7일 전 기록에 대한 주장을 오늘 잔고로 재고 있었으니 언젠가 반드시
    #    틀릴 검사였다. 바뀌는 값 위에 전제를 세우면 그렇게 된다(같은 병을
    #    감사 301에서 한 번 더 만났다).
    #
    #    그날의 **종목별 보유**는 기록에 남지 않는다(alloc은 목표 비중이다).
    #    그러니 "그날 잔고에 없었다"는 이 자리에서 확인할 수 없다 —
    #    지어내지 않는다. 대신 그날 기록 **안에서** 모순의 실체를 확인한다:
    #    필요한 돈이 가진 현금보다 컸는데도 같은 종목이 '체결'로 적혀 있다.
    #    이 사실은 앞으로도 바뀌지 않는다.
    amzn = next(c for c in rec["cash_short"] if c.get("key") == "us_stock:AMZN")
    assert float(amzn["need"]) > float(amzn["cash"]), (
        "현금이 모자랐다는 기록이 아니다 — 이 검사의 전제가 바뀌었다: "
        f"{amzn}")
