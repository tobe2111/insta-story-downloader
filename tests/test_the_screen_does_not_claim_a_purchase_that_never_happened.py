...
def test_the_real_ledger_still_contains_the_contradiction():
    ...
    # ⚠️ 예전에는 여기서 **오늘 잔고**에 아마존이 없다는 것도 함께 봤다.
    #    그런데 2026-08-20에 미국주식 종목이 늘면서 아마존을 **정말로**
    #    샀다. 그 순간부터 이 검사는 원본 코드에서 실패했고, 이 파일이
    #    지키던 변이 3건이 전부 "검사 자체 고장"으로 찍혔다 — 정상적인
    #    매수 하나가 안전장치 셋을 조용히 껐다.
    #
    #    전제를 잘못 골랐던 것이다. 이 검사가 지키는 것은 **그날 기록
    #    안의 모순**이지 오늘 계좌에 무엇이 들어 있는가가 아니다. 오늘
    #    잔고는 매일 바뀌는 값이고, 바뀌는 값 위에 전제를 세우면 언젠가
    #    반드시 깨진다(같은 병을 감사 301에서 한 번 더 만났다).
    #
    #    그래서 그날 기록 **안에서** 모순의 실체를 확인한다: 필요한 돈이
    #    가진 현금보다 컸는데도 같은 종목이 '체결'로 적혀 있다.
