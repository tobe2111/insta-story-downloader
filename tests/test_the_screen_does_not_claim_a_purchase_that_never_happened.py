...
def test_the_real_ledger_still_contains_the_contradiction():
    ...
    #    것인데, 읽은 값이 **오늘의 잔고**였다. 2026-08-22에 계좌가 아마존을
    #    정상적으로 매수하자 그 줄이 빨간불이 됐다 — 7일 전 기록에 대한
    #    주장을 오늘 잔고로 재고 있었으니 언젠가 반드시 틀릴 검사였다.
    #    그날의 종목별 보유는 기록에 남지 않으므로(alloc은 목표 비중이다)
    #    그 사실은 이 자리에서 확인할 수 없다. 지어내는 대신 뺀다.
    #
    #    이 파일이 실제로 지키는 것은 아래 화면 검사들이다: 같은 기록을 두고
    #    화면이 "매수했다"고 말하지 않는지를 진짜 브라우저로 확인한다.
