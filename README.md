...
### 코딩 없이: 자료에서 전략 뽑기 → 심사 → (원하면) 고정

```bash
# ① PDF·유튜브 링크·트레이딩뷰 스크립트에서 규칙을 뽑아 등록
python -m quant ingest 내전략.pdf --name 내전략
#    → 매일 밤 도전자로 심사(선발전·결승전)를 받고, 이기면 챔피언이 됩니다.
#    규칙을 못 찾으면 "못 찾았다"고 말합니다 — 없는 규칙을 지어내지 않습니다.

# ② 심사와 무관하게 내 전략으로 매매하고 싶다면 — 고정(pin)
python -m quant pin --market crypto --symbol BTC/USDT --name 내전략
#    → 성적표(오디션 이력·과최적화 검증)를 먼저 보여주고,
#      확인 문구를 직접 타이핑해야 고정됩니다.
python -m quant pins     # 지금 고정된 것 확인
python -m quant unpin --market crypto --symbol BTC/USDT   # 시스템 판단 복귀
```

> **전략은 당신의 것, 브레이크는 시스템의 것.** 고정해도 킬스위치·변동성
> 타깃·검증 게이트·레버리지 금지선은 그대로 걸립니다 — 고정은 "무엇을
> 사고팔지"만 바꾸고 "얼마나 크게"는 바꾸지 못합니다. 고정 중의 손익은
> 시스템 심사의 결과가 아니라는 사실이 화면에도 표시됩니다.
