 ...
 *
 * ■ 지금 덮은 범위 — **이 목록은 검사가 대조한다**
 *
 *   완역: us.html · intraday.html · futures.html · weekly.html · ml.html ·
 *         today.html · trust.html
 *   충족률 하한: index.html · paper.html
 *   일부러 안 옮김: admin.html
 *
 * 완역 페이지는 영어로 띄우면 한국어가 **하나도 안 남아야** 한다(브라우저
 * 검사가 확인한다). 충족률 하한 페이지는 아직 한국어가 남고, 그 사실을
 * 언어 버튼 옆에 적는다(지어내지 않는다) — 아래 `partial`이 그 목록이다.
 *
 * ⚠️ **이 머리말이 낡아 있었다**(2026-09-08). "오늘의 판단·기록 검증은
 *    다음 차례이고 한국어가 남는다"고 적혀 있었는데 그 둘은 이미 완역
 *    대상이었다 — 즉 사전 파일이 **자기 상태를 틀리게 말하고** 있었다.
 *    그래서 목록을 산문에서 빼내 검사가 읽을 수 있는 모양으로 적고,
 *    `tests/test_the_dictionary_knows_its_own_scope.py`가 실제 분류와
 *    대조한다. 새 페이지가 어디에도 안 들어가면 그날 CI가 먼저 말한다.
 */
(function (root) {
  "use strict";
  ...
  root.QUANT_EN = {
    ...
    strings: {
      ...
      "13F는 미국 대형 운용사가 분기마다 의무로 공개하는 보유 종목입니다. 여러 저명 투자자가 같은 미국 종목을 겹쳐 들면 약한 참고 신호가 되는데, 우리는 이걸 두 곳에 씁니다: ① 이미 모델이 사려는 종목의 확신을 조금 키우고, ② 미국 종목 선택 모델의 입력 재료로 넣습니다. 쓸지·얼마나 쓸지는 사람이 아니라 기계가 정합니다. 한국 종목엔 13F가 없어 본 계좌에는 해당되지 않습니다.":
        "A 13F is the holdings large US managers must disclose every quarter. When several famous investors overlap on the same US stock it becomes a weak reference signal, which we use in two places: (1) nudging up the conviction on a stock the model already wants to buy, and (2) feeding it as an input to the US stock-selection model. Whether and how much to use it is decided by the machine, not by hand. Korean stocks have no 13F, so the main account is unaffected.",
      "여러 저명 투자자(13F 공시)가 겹쳐 담은 미국 종목에 한해, 이미 모델이 사겠다고 한 신호를 키웁니다. **얼마나 키울지(최대 +100%)는 기계가 과거 13F 기록의 실제 성적으로 정합니다** — 증거가 없으면 중립(+15%)에 머뭅니다. 겹쳐 담기(cluster)는 약하게나마 문서화된 신호입니다. 세기를 사람이 손으로 크게 잡지 않는 이유는, 매매 로직은 기계가 증거로 개선한다는 방침 때문이고 무엇보다 13F가 도움이 되는지 아직 확인되지 않았기 때문입니다. 매수를 새로 만들지는 않습니다 — 모델이 관망(0)이면 아무리 겹쳐도, 세기가 아무리 커도 그대로 관망입니다. 실험 트랙에만 걸고, 본 계좌에는 걸지 않습니다.":
        "For US stocks that several famous investors overlap on (13F filings), it raises a signal the model already wants to act on. **How much to raise it (up to +100%) is set by the machine from how those records actually fared** — with no evidence it stays neutral (+15%). Overlap (cluster) is a weakly documented signal. We don't set the weight large by hand because trading logic is improved by the machine on evidence, and above all because it isn't yet confirmed that 13F helps. It never creates a new buy — if the model is standing pat (0), no amount of overlap, and no size of weight, changes that. It applies to the experimental track only, never the main account.",
    ...
    rules: [
      ...
      // ── 잘못됐을 때만 뜨는 경보 문구 (2026-09-08) ──────────────────
      // ⚠️ 왜 여기 없었나 — 이 줄들은 **장부에 그 사고가 적혀야만** 화면에
      //    뜬다. 브라우저 영어 검사는 살아 있는 자료로 페이지를 띄우므로,
      //    사고가 없는 날에는 이 줄 자체가 없어 "영어가 빠졌다"가 안 드러난다.
      //    실측(2026-09-08): 첫 화면 경보 32개의 텍스트 조각 70개 중 **54개**가
      //    사전에 아예 없었다 — 장부가 묵거나 사람이 개입했을 때, 정작 가장
      //    읽혀야 할 문장이 영어 독자에게 한국어로 나가고 있었다.
      // ⚠️ 숫자·이름·날짜는 열쇠에 박지 않는다(장부에서 오므로 곧 만료된다).
      ["^체결 낙관 의심$", "Fill assumption looks optimistic"],
      ["^— (.+?) 실측 불리 갭 ([\\d.-]+)bp가 가정 ([\\d.-]+)bp 초과 \\(표본 (\\d+)건\\)$",
       "— $1: the measured adverse gap of $2bp exceeds the assumed $3bp (sample of $4)"],
      ["^🚨 장부가 (\\d+)일 묵음$", "🚨 The ledger is $1 days stale"],
      ["^— 새벽 자동 기록이 멈춰 있습니다\\. 이 페이지의 숫자는 (.+?) 기준입니다$",
       "— the pre-dawn automatic recording has stopped. The figures on this page are as of $1"],
      ["^🚨 장부에 계좌보다 큰 금액$", "🚨 The ledger holds an amount larger than the account"],
      ["^— 통화 환산 누락 의심\\. 이 날의 자산·수익률 숫자도 함께 의심해야 합니다$",
       "— a missing currency conversion is suspected. Treat this day's equity and return figures with the same suspicion"],
      ["^🚨 증거금 부족으로 잘린 공매도 (\\d+)건$",
       "🚨 $1 short orders were cut for insufficient margin"],
      ["^— 장부의 목표 노출과 실제 보유가 어긋나 있습니다\\. 숏은 아직 링에 없어야 정상이라, 전략·증거금 설정 중 하나가 예상과 다릅니다$",
       "— the ledger's target exposure and the actual holdings disagree. Shorts should not be in the ring yet, so either the strategy or the margin setting differs from what we expect"],
      ["^피처 계측 교정 전 기록$", "Record predates the feature-meter fix"],
      ["^— 이 날의 피처 건강 수치는 계측기 오류\\(존재하지 않는 컬럼 이름을 셈\\)로 0으로 남아 있습니다\\. (\\d{4}-\\d{2}-\\d{2})에 고쳤고 그 이후 기록부터 유효합니다$",
       "— this day's feature-health figures are left at zero because the meter was broken (it counted column names that never existed). It was fixed on $1, and records from then on are valid"],
      ["^선택 피처 전멸$", "No optional features attached at all"],
      ["^— 이 유니버스에 붙을 수 있는 보조 지표 (\\d+)개가$",
       "— of the $1 auxiliary indicators that could attach in this universe,"],
      ["^— 보조 지표 (\\d+)개가$", "— of the $1 auxiliary indicators,"],
      ["^한 종목도$", "not one symbol"],
      ["^붙지 않았습니다\\(외부 데이터 수신 실패 가능\\)\\. 모델은 기본 피처만으로 판단 중입니다$",
       "has any attached (an external data feed may have failed). The model is judging on the base features alone"],
      ["^피처 결손$", "Feature shortfall"],
      // 아래 넷은 **같은 문장의 네 모양**이다 — 뒤 두 절이 조건부라 그렇다.
      // 한 정규식에 선택 그룹으로 담으면 안 뽑힌 절의 한국어가 그대로 남는다.
      ["^— 붙을 수 있는 보조 지표 (\\d+)개 중 (\\d+)개만 붙었습니다\\(충족률 (\\d+)%\\)$",
       "— only $2 of the $1 auxiliary indicators that could attach did (coverage $3%)"],
      ["^— 붙을 수 있는 보조 지표 (\\d+)개 중 (\\d+)개만 붙었습니다\\(충족률 (\\d+)%\\) · 가장 빈약한 종목은 (.+?)\\((\\d+)/(\\d+)\\)입니다$",
       "— only $2 of the $1 auxiliary indicators that could attach did (coverage $3%) · the thinnest symbol is $4 ($5/$6)"],
      ["^— 붙을 수 있는 보조 지표 (\\d+)개 중 (\\d+)개만 붙었습니다\\(충족률 (\\d+)%\\) · 전 종목 누락: (.+)$",
       "— only $2 of the $1 auxiliary indicators that could attach did (coverage $3%) · missing on every symbol: $4"],
      ["^— 붙을 수 있는 보조 지표 (\\d+)개 중 (\\d+)개만 붙었습니다\\(충족률 (\\d+)%\\) · 가장 빈약한 종목은 (.+?)\\((\\d+)/(\\d+)\\)입니다 · 전 종목 누락: (.+)$",
       "— only $2 of the $1 auxiliary indicators that could attach did (coverage $3%) · the thinnest symbol is $4 ($5/$6) · missing on every symbol: $7"],
      ["^실적 가드가 지금 아무것도 지키지 못합니다$",
       "The earnings guard is protecting nothing right now"],
      ["^— 발표일을 물어본 (\\d+)종목 가운데$", "— of the $1 symbols whose earnings dates we asked for,"],
      ["^날짜를 아는 종목이 하나도 없습니다$", "not one has a known date"],
      ["^\\. 실적 발표 전후는 하루 만에 가격이 크게 뛰는 날이라, 그날 비중을 줄이는 장치가 꺼져 있다는 뜻입니다$",
       ". Prices can jump hard around an earnings release, so the device that trims exposure on those days is effectively off"],
      ["^\\. 실적 발표 전후는 하루 만에 가격이 크게 뛰는 날이라, 그날 비중을 줄이는 장치가 꺼져 있다는 뜻입니다 \\(조회 오류: (.+)\\)$",
       ". Prices can jump hard around an earnings release, so the device that trims exposure on those days is effectively off (lookup errors: $1)"],
      ["^실적 발표일 조회 실패 (\\d+)종목$", "Earnings-date lookup failed on $1 symbols"],
      ["^— (.+?)\\. 그 종목은 발표일을 모르는 채로 굴러갑니다$",
      ["^배분 폴백$", "Allocation fell back"],
      ["^— 상관 추정이 안 돼 위험 배분\\(HRP·ERC\\)이 자본 균등 1/n으로 내려갔습니다$",
       "— the correlation estimate failed, so risk-based allocation (HRP, ERC) dropped to equal capital weights (1/n)"],
      ["^— HRP가 실패해 ERC\\(위험기여 균등\\)로 내려갔습니다$",
       "— HRP failed, so it dropped to ERC (equal risk contribution)"],
      ["^— 배정 예산이 1주 값에 못 미쳐 담기지 못했습니다\\((.+?)\\)\\. 분산 종목 수가 표시보다 적습니다$",
       "— the budget allotted came to less than one share, so they could not be bought ($1). The number of symbols actually diversifying across is smaller than shown"],
      ["^예산을 끌어 쓴 종목 (\\d+)개$", "$1 symbols that borrowed budget"],
      ["^— (.+?)\\. 확신도가 높은 쪽부터 채우기 때문이며, 그만큼 종목 수는 줄어듭니다$",
       "— $1. Budget is filled from the highest-conviction name down, and the symbol count shrinks by that much"],
      ["^— (.+?)\\. 대신 (.+?)이\\(가\\) 자리를 내줬습니다\\. 확신도가 높은 쪽부터 채우기 때문이며, 그만큼 종목 수는 줄어듭니다$",
       "— $1. In exchange, $2 gave up their place. Budget is filled from the highest-conviction name down, and the symbol count shrinks by that much"],
      ["^🚨 금액이 계좌와 안 맞는 기록 (\\d+)건$",
       "🚨 $1 ledger entries whose amounts do not fit the account"],
      ["^ \\((.+?)\\) — 그날 자산 (.+?)보다 큰 금액이 적혀 있어$",
       " ($1) — an amount larger than that day's equity of $2 is recorded, so"],
      ["^화면에 표시하지 않습니다$", "we do not display it"],
      ["^\\. 레버리지가 잠긴 계좌에서는 성립할 수 없는 숫자이고, 통화 환산이 빠졌을 때 생깁니다\\. 기록은 지우지 않고 그대로 두었습니다 —$",
       ". In an account with leverage locked such a number cannot arise; it appears when a currency conversion is skipped. The record is left in place, not deleted —"],
      ["^에 무슨 일이었는지 적어 두었습니다$", " says what happened"],
      ["^🚨 (.+?) 부분 실패 (\\d+)종목$", "🚨 $1 partly failed on $2 symbols"],
      ["^— (.+?)\\. 그 종목은 그날 판단·기록이 없고 이전 판단을 그대로 씁니다$",
       "— $1. Those symbols have no judgement or record for that day and carry the previous judgement forward"],
      ["^— (.+?)\\. 그 종목은 그날 판단·기록이 없고 이전 판단을 그대로 씁니다 \\((.+?)\\)$",
       "— $1. Those symbols have no judgement or record for that day and carry the previous judgement forward ($2)"],
      ["^🚨 (.+?) 정체 (\\d+)종목$", "🚨 $1 stalled on $2 symbols"],
      ["^— 최장 (.+?)째 새 봉을 못 받아 건너뛰고 있습니다\\((.+?)\\)\\. 주말·휴장으로는 설명되지 않는 간격입니다$",
       "— the longest has gone $1 without a new bar and is being skipped ($2). The gap is not explained by weekends or market holidays"],
      ["^🚨 묵은 봉으로 판단 (\\d+)개 시장$", "🚨 Judging on stale bars in $1 markets"],
      ["^— (.+?)\\. 그 시장의 오늘 판단은 최신 시세를 못 본 것이고, 다른 시장과 같은 날로 비교할 수 없습니다$",
       "— $1. Today's judgement in those markets never saw the latest prices, and cannot be compared with other markets as the same day"],
      ["^묵은 가격 평가 (\\d+)종목$", "$1 symbols valued at stale prices"],
      ["^— 새 시세를 못 받아 직전 가격으로 평가했습니다\\. 이 날의 손익은 확정값이 아닙니다$",
       "— no fresh quote arrived, so they were valued at the previous price. This day's profit and loss is not a settled figure"],
      ["^이 날의 기록은 나중에 되돌렸습니다$", "This day's record was rolled back later"],
      ["^— (.+?) \\(되돌리기 전 자산 ([\\d,]+)원\\)$", "— $1 (equity before the rollback: KRW $2)"],
      ["^표본이 모자란 판단 (\\d+)종목$", "$1 symbols judged on a short sample"],
      ["^— (.+?)\\. 데이터를 요청한 만큼 받지 못했습니다\\. 이 종목의 성적과 검증 수치는 표본 부족의 산물일 수 있습니다$",
       "— $1. We did not receive as much data as we asked for. Those symbols' results and validation figures may be an artefact of the short sample"],
      ["^현금 부족으로 거부된 주문 (\\d+)건$", "$1 orders refused for want of cash"],
      ["^— (.+?)\\. 계획한 만큼 사지 못했습니다\\. 이 날의 비중은 전략이 정한 값이 아닙니다$",
       "— $1. We could not buy as much as planned. This day's weights are not the ones the strategy chose"],
      ["^자릿수가 안 맞아 거부된 체결 (\\d+)건$", "$1 fills refused because the magnitudes did not match"],
      ["^— (.+?)\\. 체결가와 평가가격이 같은 통화가 아닌 것으로 보여 주문을 막았습니다$",
       "— $1. The fill price and the valuation price did not look like the same currency, so the order was blocked"],
      ["^합성 데이터 폴백 (\\d+)종목$", "$1 symbols fell back to synthetic data"],
      ["^— 실제 시세를 받지 못했습니다\\((.+?)\\)\\. 이 종목의 그날 기록은 실제 시장이 아닙니다$",
       "— no real quote arrived ($1). Those symbols' records for that day are not the real market"],
      ["^사람의 개입 — 일시정지$", "Human intervention — paused"],
      ["^— 운영자가 신규 주문을 멈춘 상태입니다\\(보유는 유지\\)\\. 이 날의 성적은 전략만의 결과가 아닙니다$",
       "— the operator has stopped new orders (holdings are kept). This day's result is not the strategy's alone"],
      ["^사람의 개입 — 노출 배수 (\\d+)%$", "Human intervention — exposure multiplier $1%"],
      ["^— 운영자가 손으로 위험 크기를 조정했습니다$", "— the operator adjusted the risk size by hand"],
       "— $1. Those symbols run without knowing their earnings dates"],
